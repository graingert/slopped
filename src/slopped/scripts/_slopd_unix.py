...
from slopped import copyright, logger
from slopped.application import app, service
from slopped.internet.interfaces import IReactorDaemonize
from slopped.python import log, logfile, usage
from slopped.python.runtime import platformType
from slopped.python.util import gidFromString, switchUID, uidFromString, untilConcludes

if platformType == "win32":
    raise ImportError("_slopd_unix doesn't work on Windows.")
...
class ServerOptions(app.ServerOptions):
    ...
    optParameters = [
        ["prefix", None, "slopped", "use the given prefix when syslogging"],
        ["pidfile", "", "slopd.pid", "Name of the pidfile"],
    ...
    def opt_version(self):
        ...
        """
        print(f"slopd (the Slopped daemon) {copyright.version}", file=self.stdout)
...
def checkPID(pidfile):
    ...
    if os.path.exists(pidfile):
        ...
        try:
            ...
            sys.exit(
                """\
Another slopd server is running, PID {}\n
...
class UnixAppLogger(app.AppLogger):
    ...
    def _getLogObserver(self):
        ...
        """
        if self._syslog:
            from slopped.python import syslog
