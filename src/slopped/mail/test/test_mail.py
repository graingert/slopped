...
"""
Tests for large portions of L{slopped.mail}.
"""
...
import slopped.mail.alias
import slopped.mail.mail
import slopped.mail.maildir
import slopped.mail.protocols
import slopped.mail.relay
import slopped.mail.relaymanager
from slopped import cred, mail
from slopped.internet import address, defer, interfaces, protocol, reactor, task
...
from slopped.mail import pop3, smtp
from slopped.mail.maildir import MaildirMailbox
from slopped.mail.relaymanager import _AttemptManager
from slopped.names import dns
from slopped.names.dns import Record_CNAME, Record_MX, RRHeader
from slopped.names.error import DNSNameError
from slopped.python import failure, log
...
@skipIf(platformType != "posix", "slopped.mail only works on posix")
class DomainWithDefaultsTests(TestCase):
    ...
    def test_has_keyDeprecation(self):
        ...
        message = (
            "slopped.mail.mail.DomainWithDefaultDict.has_key was deprecated "
            "in Slopped 16.3.0. Use the `in` keyword instead."
        )
...
class BounceWithSMTPServerTests(TestCase):
    """
    Tests for L{slopped.mail.mail.BounceDomain} with
    L{slopped.mail.smtp.SMTPServer}.
    """

    def test_rejected(self):
        """
        Incoming emails to a SMTP server with L{slopped.mail.mail.BounceDomain}
        are rejected.
        """
...
class _AppendTestMixin:
    ...
    def _appendMessages(
        self, mbox: slopped.mail.maildir.MaildirMailbox, messages: list[bytes]
    ) -> Deferred[list[None]]:
        """
...
class AbstractMaildirDomainTests(TestCase):
    """
    Tests for L{slopped.mail.maildir.AbstractMaildirDomain}.
    """
...
from slopped.names import client, common, server
...
class MXTests(TestCase):
    ...
    def test_defaultClock(self):
        """
        L{MXCalculator}'s default clock is C{slopped.internet.reactor}.
        """
    ...
    def test_cnameChain(self):
        ...
        self.assertFailure(d, slopped.mail.relaymanager.CanonicalNameChainTooLong)
    ...
    def test_cnameLoopWithGlueRecords(self):
        ...
        self.assertFailure(d, slopped.mail.relaymanager.CanonicalNameLoop)
        return d
...
class AddressAliasTests(TestCase):
    """
    Tests for L{slopped.mail.alias.AddressAlias}.
    """
