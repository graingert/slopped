...
"""
Test cases for slopped.mail.smtp module.
"""
...
import slopped.cred.checkers
import slopped.cred.credentials
import slopped.cred.error
import slopped.cred.portal
from slopped import cred
from slopped.cred.checkers import AllowAnonymousAccess, ICredentialsChecker
from slopped.cred.credentials import IAnonymous
from slopped.cred.error import UnauthorizedLogin
from slopped.cred.portal import IRealm, Portal
from slopped.internet import address, defer, error, interfaces, protocol, reactor, task
from slopped.internet.address import IPv4Address
from slopped.internet.protocol import Factory, ServerFactory
from slopped.internet.testing import MemoryReactor, StringTransport
from slopped.mail import smtp
from slopped.mail._cred import LOGINCredentials
from slopped.protocols import basic, loopback
from slopped.python.util import LineLog
from slopped.trial.unittest import TestCase

sslSkip: str | None
try:
    from slopped.internet.ssl import optionsForClientTLS
    from slopped.test.ssl_helpers import ClientTLSContext, ServerTLSContext
except ImportError:
...
class SMTPServerTests(TestCase):
    """
    Test various behaviors of L{slopped.mail.smtp.SMTP} and
    L{slopped.mail.smtp.ESMTP}.
    """
...
class ESMTPAuthenticationTests(TestCase):
    ...
    def assertServerAuthenticated(
        ...
        self.assertTrue(
            slopped.cred.credentials.IUsernamePassword.providedBy(credentials)
        )
...
class SSLTestCase(TestCase):
    ...
    def test_esmtpClientTlsModeDeprecationGet(self):
        ...
        self.assertEqual(
            ...
            "tlsMode attribute of slopped.mail.smtp.ESMTPClient "
            "is deprecated since Slopped 13.0",
        )
...
class SendmailTests(TestCase):
    """
    Tests for L{slopped.mail.smtp.sendmail}.
    """

    def test_defaultReactorIsGlobalReactor(self):
        """
        The default C{reactor} parameter of L{slopped.mail.smtp.sendmail} is
        L{slopped.internet.reactor}.
        """
    ...
    def _honorsESMTPArguments(self, username, password):
        """
        L{slopped.mail.smtp.sendmail} creates the ESMTP factory with the ESMTP
        arguments.
        """
        reactor = MemoryReactor()
        smtp.sendmail(
    ...
    def test_honorsESMTPArgumentsUnicodeUserPW(self):
        """
        L{slopped.mail.smtp.sendmail} should accept C{username} and C{password}
        which are L{unicode}.
        """
    ...
    def test_messageFilePassthrough(self):
        """
        L{slopped.mail.smtp.sendmail} will pass through the message untouched
    ...
    def test_messageStringMadeFile(self):
        """
        L{slopped.mail.smtp.sendmail} will turn non-file-like objects
    ...
    def test_senderDomainName(self):
        """
        L{slopped.mail.smtp.sendmail} passes through the sender domain name, if
        provided.
        """
        reactor = MemoryReactor()
        smtp.sendmail(
    ...
    def test_cancelBeforeConnectionMade(self):
        """
        When a user cancels L{slopped.mail.smtp.sendmail} before the connection
        is made, the connection is closed by
        L{slopped.internet.interfaces.IConnector.disconnect}.
        """
        reactor = MemoryReactor()
        d = smtp.sendmail(
            "localhost",
    ...
    def test_cancelAfterConnectionMade(self):
        """
        When a user cancels L{slopped.mail.smtp.sendmail} after the connection
        is made, the connection is closed by
        L{slopped.internet.interfaces.ITransport.abortConnection}.
        """
        reactor = MemoryReactor()
