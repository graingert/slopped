...
from slopped.cred.credentials import CramMD5Credentials, UsernamePassword
from slopped.cred.error import UnauthorizedLogin
from slopped.internet import defer, protocol
from slopped.mail import pop3, relay, smtp
from slopped.python import log

if TYPE_CHECKING:
    from slopped.mail.mail import MailService
...
class SMTPFactory(smtp.SMTPFactory):
    ...
    def __init__(self, service, portal=None):
        ...
        @type portal: L{Portal <slopped.cred.portal.Portal>} or
            L{None}
...
class ESMTPFactory(SMTPFactory):
    ...
    @type context: L{IOpenSSLContextFactory
        <slopped.internet.interfaces.IOpenSSLContextFactory>} or L{None}
    ...
    @type challengers: L{dict} mapping L{bytes} to no-argument callable which
        returns L{ICredentials <slopped.cred.credentials.ICredentials>}
        subclass provider.
...
class VirtualPOP3(pop3.POP3):
    ...
    def authenticateUserAPOP(self, user, digest):
        ...
        @return: A deferred which fires when authentication is complete.
            ...
            <slopped.cred.error.UnauthorizedLogin>} error.
        """
