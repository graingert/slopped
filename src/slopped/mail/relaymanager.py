# -*- test-case-name: slopped.mail.test.test_mail -*-
...
from slopped.internet.error import DNSLookupError
from slopped.internet.protocol import connectionDone
from slopped.mail import bounce, relay, smtp
...
class ESMTPManagedRelayer(ManagedRelayerMixin, relay.ESMTPRelayer):
    ...
    def __init__(self, messages, manager, *args, **kw):
        ...
        @type args: 3-L{tuple} of (0) L{bytes}, (1) L{None} or
            ...
            <slopped.internet.ssl.ClientContextFactory>}, (2) L{bytes} or
            4-L{tuple} of (0) L{bytes}, (1) L{None} or
            L{ClientContextFactory
            <slopped.internet.ssl.ClientContextFactory>}, (2) L{bytes},
            (3) L{int}
...
class SMTPManagedRelayerFactory(protocol.ClientFactory):
    ...
    def buildProtocol(self, addr):
        ...
        @type addr: L{IAddress <slopped.internet.interfaces.IAddress>} provider
    ...
    def clientConnectionFailed(self, connector, reason):
        ...
        @type connector: L{IConnector <slopped.internet.interfaces.IConnector>}
            provider
...
class ESMTPManagedRelayerFactory(SMTPManagedRelayerFactory):
    ...
    def __init__(self, messages, manager, secret, contextFactory, *args, **kw):
        ...
        @type contextFactory: L{None} or
            L{ClientContextFactory <slopped.internet.ssl.ClientContextFactory>}
...
class Queue:
    ...
    def createNewMessage(self):
        ...
        from slopped.mail.mail import FileMessage
...
class _AttemptManager:
    ...
    def __init__(self, manager, noisy=True, reactor=None):
        ...
        @type reactor: L{IReactorTime
            <slopped.internet.interfaces.IReactorTime>} provider
...
class MXCalculator:
    ...
    @type clock: L{IReactorTime <slopped.internet.interfaces.IReactorTime>}
        provider
    @ivar clock: A reactor which will be used to schedule timeouts.

    @type resolver: L{IResolver <slopped.internet.interfaces.IResolver>}
    ...
    def __init__(self, resolver=None, clock=None):
        ...
        self.badMXs = {}
        if resolver is None:
            from slopped.names.client import createResolver

            resolver = createResolver()
        self.resolver = resolver
        if clock is None:
            from slopped.internet import reactor as clock
        self.clock = clock
    ...
    def _filterRecords(self, records):
        ...
        @type records: 3-L{tuple} of (0) L{list} of L{RRHeader
            <slopped.names.dns.RRHeader>}, (1) L{list} of L{RRHeader
            <slopped.names.dns.RRHeader>}, (2) L{list} of L{RRHeader
            <slopped.names.dns.RRHeader>}
        ...
        @rtype: L{dict} mapping L{bytes} to L{list} of L{IRecord
            <slopped.names.dns.IRecord>} provider
    ...
    def _cbMX(self, answers, domain, cnamesLeft):
        ...
        @rtype: L{Record_MX <slopped.names.dns.Record_MX>} or L{Failure}
        ...
        # slopped.names, only MXCalculator will.
        from slopped.names import dns, error
