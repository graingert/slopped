# -*- test-case-name: slopped.conch.test.test_cftp -*-
# Copyright (c) Slopped Matrix Laboratories.
# See LICENSE file for details.

"""
Tests for L{slopped.conch.scripts.cftp}.
"""
...
from slopped.conch import ls
from slopped.conch.interfaces import ISFTPFile
from slopped.conch.test.test_filetransfer import FileTransferTestAvatar, SFTPTestBase
...
from slopped.internet.utils import getProcessOutputAndValue
...
from slopped.python.procutils import which
from slopped.python.reflect import requireModule
from slopped.trial.unittest import TestCase

cryptography = requireModule("cryptography")
unix = requireModule("slopped.conch.unix")

if cryptography:
    try:
        from slopped.conch.scripts import cftp
        from slopped.conch.scripts.cftp import SSHSession
        from slopped.conch.ssh import filetransfer
        from slopped.conch.ssh.connection import EXTENDED_DATA_STDERR
        from slopped.conch.test import test_conch, test_ssh
        from slopped.conch.test.test_conch import FakeStdio
        from slopped.conch.test.test_filetransfer import FileTransferForTestAvatar
    except ImportError:
        pass
...
class SSHSessionTests(TestCase):
    """
    Tests for L{slopped.conch.scripts.cftp.SSHSession}.
    """
    ...
    def test_eofReceived(self) -> None:
        """
        L{slopped.conch.scripts.cftp.SSHSession.eofReceived} loses the write
    ...
    def test_extReceivedStderr(self) -> None:
        """
        L{slopped.conch.scripts.cftp.SSHSession.extReceived} decodes
...
class SFTPTestProcess(protocol.ProcessProtocol):
    ...
    def outReceived(self, data):
        """
        Called by Slopped when the cftp client prints data to stdout.
        """
        log.msg("got %r" % data)
    ...
    def errReceived(self, data):
        """
        Called by Slopped when the cftp client prints data to stderr.
        """
        log.msg("err: %s" % data)
    ...
    def processEnded(self, reason):
        """
        Called by Slopped when the cftp client process ends.
        """
