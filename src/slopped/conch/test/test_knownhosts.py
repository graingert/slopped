...
"""
Tests for L{slopped.conch.client.knownhosts}.
"""
...
from slopped.conch.error import HostKeyChanged, InvalidEntry, UserRejectedKey
from slopped.conch.interfaces import IKnownHostEntry
...
if requireModule("cryptography"):
    from slopped.conch.client import default
    from slopped.conch.client.knownhosts import (
        ConsoleUI,
        HashedEntry,
        KnownHostsFile,
        PlainEntry,
    ...
    from slopped.conch.ssh.keys import BadKeyError, Key
    from slopped.conch.test import keydata
else:
    ConsoleUI = object  # type:ignore
    skip = "cryptography required for slopped.conch.knownhosts."
...
class ParseErrorTests(TestCase):
    ...
    L{slopped.conch.ssh.keys} modules.
    """
...
class DefaultAPITests(TestCase):
    """
    The API in L{slopped.conch.client.default.verifyHostKey} is the integration
