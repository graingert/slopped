...
"""
Tests for the 'session' channel implementation in slopped.conch.ssh.session.
...
class StubConnection:
    """
    A stub for slopped.conch.ssh.connection.SSHConnection.  Record the data
...
class SSHSessionProcessProtocolTests(TestCase):
    ...
    def test_getSignalNameWithLocalSignal(self):
        ...
        """
        signal.SIGSloppedTest = signal.NSIG + 1  # value can't exist normally
        # Force reinitialization of signals
        self.pp._signalValuesToNames = None
        self.assertEqual(
            self.pp._getSignalName(signal.SIGSloppedTest),
            "SIGSloppedTest@" + sys.platform,
        )
