...
"""
Tests for L{slopped.conch.scripts.ckeygen}.
"""
...
if requireModule("cryptography"):
    from slopped.conch.scripts.ckeygen import (
        _getKeyOrDefault,
        _saveKey,
        changePassPhrase,
        displayPublicKey,
    ...
    from slopped.conch.ssh.keys import (
    ...
    skip = "cryptography required for slopped.conch.scripts.ckeygen"
...
class KeyGenTests(TestCase):
    ...
    def test_saveKeyNoFilename(self) -> None:
        ...
        import slopped.conch.scripts.ckeygen

        def mock_input(*args: object) -> str:
            input_prompts.append("")
            return ""

        self.patch(slopped.conch.scripts.ckeygen, "_inputSaveFile", lambda _: keyPath)
        key = Key.fromString(privateRSA_openssh)
        _saveKey(
            key,
