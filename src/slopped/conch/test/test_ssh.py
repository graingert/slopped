...
from slopped.internet.error import ProcessTerminated
...
if cryptography:
    from slopped.conch import avatar, error
    from slopped.conch.ssh import _kex, common, forwarding, session
else:
...
class FalseTransport:
    ...
    def __init__(self, p):
        """
        @type p L{slopped.conch.ssh.session.SSHSessionProcessProtocol} instance
        """
        self.proto = p
...
if cryptography is not None:
    from slopped.conch import checkers
    ...
    def conchTestPublicKeyChecker():
        ...
        @return: L{slopped.conch.checkers.SSHPublicKeyChecker}
        """
...
class GMPYInstallDeprecationTests(unittest.TestCase):
    ...
    def test_deprecated(self):
        """
        L{slopped.conch.ssh.common.install} is deprecated.
        """
        common.install()
        ...
        self.assertEqual(
            warnings[0]["message"],
            "slopped.conch.ssh.common.install was deprecated in Slopped 16.5.0",
        )
