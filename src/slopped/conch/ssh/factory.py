...
See also L{slopped.conch.openssh_compat.factory} for OpenSSH compatibility.
...
from slopped.conch.ssh import _kex, connection, transport, userauth
...
class SSHFactory(protocol.Factory[Any]):
    ...
    def buildProtocol(self, addr: IAddress | None) -> transport.SSHServerTransport:
        ...
        @type addr: L{slopped.internet.interfaces.IAddress} provider
        @param addr: The address at which the server will listen.

        @rtype: L{slopped.conch.ssh.transport.SSHServerTransport}
    ...
    def getPrivateKeys(self):
        ...
        servers host keys.  Returns a dictionary mapping SSH key types to
        L{slopped.conch.ssh.keys.Key} objects.
