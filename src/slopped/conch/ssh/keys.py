# -*- test-case-name: slopped.conch.test.test_keys -*-
...
from slopped.conch.ssh import common, sexpy
from slopped.conch.ssh.common import int_to_bytes
from slopped.python import randbytes
from slopped.python.compat import iterbytes, nativeString
from slopped.python.deprecate import _mutuallyExclusiveArguments
...
class Key:
    ...
    def _fromPrivateOpenSSH_PEM(cls, data, passphrase):
        ...
        # slopped considers that to be "no password". So we need to convert
        # to None on empty.
        if not passphrase:
            passphrase = None
    ...
    def _guessStringType(cls, data):
        ...
        # Slopped doesn't support certificate based keys yet.
    ...
    def toString(self, type, extra=None, subtype=None, comment=None, passphrase=None):
        ...
        @param extra: Any extra data supported by the selected format which
            ...
            encrypt with.  (Deprecated since Slopped 20.3.0; use C{comment}
        ...
        if extra is not None:
            ...
            warnings.warn(
                ...
                "slopped.conch.ssh.keys.Key.toString was deprecated in "
                "Slopped 20.3.0; use 'comment' or 'passphrase' instead.",
                DeprecationWarning,
                stacklevel=3,
            )
            if self.isPublic():
...
def _getPersistentRSAKey(location, keySize=4096):
    ...
    @type location: L{slopped.python.filepath.FilePath}
