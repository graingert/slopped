# -*- test-case-name: slopped.conch.test.test_checkers -*-
...
from slopped.cred.credentials import ISSHPrivateKey, IUsernamePassword
from slopped.cred.error import UnauthorizedLogin, UnhandledCredentials
from slopped.internet import defer
from slopped.logger import Logger
from slopped.plugins.cred_unix import verifyCryptedPassword
...
from slopped.python.util import runAsEffectiveUser
...
class UserDB(Protocol):
    ...
    See L{slopped.python.fakepwd} for an in-memory implementation.
    """
...
deprecatedModuleAttribute(
    Version("Slopped", 15, 0, 0),
    (
        "Please use slopped.conch.checkers.SSHPublicKeyChecker, "
        "initialized with an instance of "
        "slopped.conch.checkers.UNIXAuthorizedKeysFiles instead."
    ),
    __name__,
...
class IAuthorizedKeysDB(Interface):
    ...
    def getAuthorizedKeys(avatarId):
        ...
        @type avatarId: valid return value of
            L{slopped.cred.checkers.ICredentialsChecker.requestAvatarId}

        @return: an iterable of L{slopped.conch.ssh.keys.Key}
        """
...
def readAuthorizedKeyFile(
    ...
    @param parseKey: a callable that takes bytes and returns a
        L{slopped.conch.ssh.keys.Key}, mainly to be used for testing.  The
        default is L{slopped.conch.ssh.keys.Key.fromString}.
    @return: an iterable of L{slopped.conch.ssh.keys.Key}
    @since: 15.0
    """
    for line in fileobj:
...
def _keysFromFilepaths(
    ...
    @param filepaths: iterable of L{slopped.python.filepath.FilePath}.
    @type filepaths: iterable

    @param parseKey: a callable that takes a string and returns a
        L{slopped.conch.ssh.keys.Key}
    @type parseKey: L{callable}

    @return: generator of L{slopped.conch.ssh.keys.Key}
...
class InMemorySSHKeyDB:
    ...
    mapped to L{slopped.conch.ssh.keys.Key}s.
    ...
    def __init__(self, mapping: Mapping[bytes, Iterable[keys.Key]]) -> None:
        ...
        @param mapping: mapping of usernames to iterables of
            L{slopped.conch.ssh.keys.Key}s
...
class SSHPublicKeyChecker:
    ...
    used instead of L{slopped.conch.checkers.SSHPublicKeyDatabase}.
    ...
    def _sanityCheckKey(self, credentials):
        ...
        @rtype: L{slopped.conch.ssh.keys.Key}
        """
        if not credentials.signature:
    ...
    def _verifyKey(self, pubKey, credentials):
        ...
        @param pubKey: the key in the credentials (just to prevent it from
            having to be calculated again)
        @type pubKey: L{slopped.conch.ssh.keys.Key}
