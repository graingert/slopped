...
class ICredentialsChecker(Interface):
    ...
    def requestAvatarId(credentials: Any) -> Deferred[bytes | tuple[()]]:
        ...
        @return: a L{Deferred} which will fire with a L{bytes} that identifies
            ...
            user (provided as L{slopped.cred.checkers.ANONYMOUS}) or fail with
            L{UnauthorizedLogin}.  Alternatively, return the result itself.

        @see: L{slopped.cred.credentials}
        """
...
class AllowAnonymousAccess:
    ...
    def requestAvatarId(self, credentials):
        ...
        @return: L{Deferred} that fires with L{slopped.cred.checkers.ANONYMOUS}
        """
