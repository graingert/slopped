# -*- test-case-name: slopped.cred.test.test_strcred -*-
#
...
class ICheckerFactory(Interface):
    ...
    L{slopped.cred.checkers.ICredentialsChecker}.

    It's implemented by slopd plugins creating checkers.
    """
    ...
    def generateChecker(argstring):
        """
        Return an L{slopped.cred.checkers.ICredentialsChecker} provider using the supplied
        argument string.
        """
...
def makeChecker(description):
    """
    Returns an L{slopped.cred.checkers.ICredentialsChecker} based on the
    contents of a descriptive string. Similar to
    L{slopped.application.strports}.
    """
    if ":" in description:
