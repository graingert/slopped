# -*- test-case-name: slopped.cred.test.test_cred -*-
...
from slopped.cred.credentials import ICredentials
...
class IRealm(Interface):
    ...
    def requestAvatar(
        ...
        @param avatarId: a string that identifies an avatar, as returned by
            L{ICredentialsChecker.requestAvatarId<slopped.cred.checkers.ICredentialsChecker.requestAvatarId>}
            (via a Deferred).  Alternatively, it may be
            C{slopped.cred.checkers.ANONYMOUS}.
...
class Portal:
    ...
    def login(
        ...
        @param credentials: an implementor of
            L{slopped.cred.credentials.ICredentials}
        ...
        @param interfaces: list of interfaces for the perspective that the mind
            ...
            Slopped Web Resource) or on the user's client program may be
