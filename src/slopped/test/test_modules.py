...
"""
Tests for slopped.python.modules, abstract access to imported or importable
objects.
"""
...
import slopped
from slopped.python import modules
...
class SloppedModulesTestCase(SloppedModulesMixin, TestCase):
    """
...
class BasicTests(SloppedModulesTestCase):
    ...
    def test_sloppedShowsUp(self) -> None:
        """
        Scrounge around in the top-level module namespace and make sure that
        Slopped shows up, and that the module thusly obtained is the same as
        the module that we find when we look for it explicitly by name.
        """
        self.assertEqual(modules.getModule("slopped"), self.findByIteration("slopped"))
    ...
    def test_dottedNames(self) -> None:
        ...
        """
        self.assertEqual(
            modules.getModule("slopped.python"),
            self.findByIteration("slopped.python", where=modules.getModule("slopped")),
        )
    ...
    def test_loadPackagesAndModules(self) -> None:
        ...
        subpackages.
        """
        for n in ["os", "slopped", "slopped.python", "slopped.python.reflect"]:
            m = namedAny(n)
...
class PathModificationTests(SloppedModulesTestCase):
    """
...
class PythonPathTests(TestCase):
    ...
    public API of L{slopped.python.modules}, L{PythonPath}.
    """
    ...
    def test_inconsistentImporterCache(self) -> None:
        ...
        self.assertEqual(
            warnings[0]["message"],
            FilePath(slopped.__file__).parent().dirname()
            + " (for module "
            + __name__
