...
"""
Tests for L{slopped.application.app} and L{slopped.scripts.slopd}.
"""
...
from slopped import internet, logger, plugin
from slopped.application import app, reactors, service
...
from slopped.internet.interfaces import IReactorDaemonize, _ISupportsExitSignalCapturing
from slopped.internet.test.modulehelpers import AlternateReactor
from slopped.internet.testing import MemoryReactor
from slopped.logger import ILogObserver, globalLogBeginner, globalLogPublisher
from slopped.python import util
from slopped.python.components import Componentized
from slopped.python.fakepwd import UserDatabase
from slopped.python.log import ILogObserver as LegacyILogObserver, textFromEventDict
from slopped.python.reflect import requireModule
from slopped.python.runtime import platformType
from slopped.python.usage import UsageError
from slopped.scripts import slopd
from slopped.test.test_process import MockOS
from slopped.trial.unittest import TestCase

_slopd_unix = requireModule("slopped.scripts._slopd_unix")
if _slopd_unix:
    from slopped.scripts._slopd_unix import (
...
syslog = requireModule("slopped.python.syslog")
...
class MockServiceMaker:
    """
    A non-implementation of L{slopped.application.service.IServiceMaker}.
    """
...
class CrippledApplicationRunner(slopd._SomeApplicationRunner):
    """
...
class ServerOptionsTests(TestCase):
    ...
    def test_sortedReactorHelp(self):
        ...
        class FakeReactorInstaller:
            def __init__(self, name):
                ...
                self.description = "description of " + name
                self.moduleName = "slopped.internet.default"
    ...
    @skipIf(not _slopd_unix, "slopd unix not available")
    ...
    def test_badAttributeWithConfiguredLogObserver(self):
        ...
        e = self.assertRaises(
            UsageError,
            config.parseOptions,
            ["--logger", "slopped.test.test_slopd.FOOBAR"],
        )
        self.assertTrue(
            e.args[0].startswith(
                "Logger 'slopped.test.test_slopd.FOOBAR' could not be "
                "imported: module 'slopped.test.test_slopd' "
    ...
    def test_version(self):
        ...
        if platformType == "win32":
            name = "(the Slopped Windows runner)"
        else:
            name = "(the Slopped daemon)"
        expectedOutput = "slopd {} {}\n{}\n".format(
            name, copyright.version, copyright.copyright
        )

        stdout = StringIO()
        config = slopd.ServerOptions(stdout=stdout)
...
class CheckPIDTests(TestCase):
    ...
    def test_anotherRunning(self):
        """
        Another running slopd server causes a system exit.
        """
        pidfile = self.mktemp()
        with open(pidfile, "w") as f:
            f.write("42")
        ...
        e = self.assertRaises(SystemExit, checkPID, pidfile)
        self.assertIn("Another slopd server", e.code)
...
class TapFileTests(TestCase):
    """
    Test slopd-related functionality that requires a tap file on disk.
    """
    ...
    def test_createOrGetApplicationWithTapFile(self):
        """
        Ensure that the createOrGetApplication call that 'slopd -f foo.tap'
...
class ApplicationRunnerTests(TestCase):
    ...
    def test_applicationRunnerGetsCorrectApplication(self):
        """
        Ensure that a slopd plugin gets used in appropriate ways: it
    ...
    def _applicationStartsWithConfiguredID(self, argv, uid, gid):
        ...
        class FakeUnixApplicationRunner(slopd._SomeApplicationRunner):
    ...
    @skipIf(
        ...
        "Platform does not support --uid/--gid slopd options.",
    )
...
class UnixApplicationRunnerSetupEnvironmentTests(TestCase):
    ...
    @ivar daemon: A boolean indicating whether daemonization has been performed
        by a call to L{_slopd_unix.daemonize} (patched for this TestCase with
    ...
    def setUp(self):
        ...
        self.runner = UnixApplicationRunner(slopd.ServerOptions())
...
class UnixApplicationRunnerStartApplicationTests(TestCase):
    ...
    def test_shedPrivileges(self):
        ...
        self.patch(_slopd_unix, "switchUID", switchUIDPass)
    ...
    def test_shedPrivilegesError(self):
        ...
        L{slopped.scripts._slopd_unix.shedPrivileges}
        ...
        runner = UnixApplicationRunner({})
        self.patch(_slopd_unix, "switchUID", switchUIDFail)
...
class AppLoggerTests(TestCase):
    ...
    def _checkObserver(self, observer):
        """
        Ensure that initial C{slopd} logs are written to logs.
    ...
    def test_start(self):
        ...
        writes some messages about slopd and the reactor.
        """
    ...
    def test_startUsesConfiguredLogObserver(self):
        ...
        (i.e., when C{--logger} is passed to slopd), the initial log observer
    ...
    def test_unmarkedObserversDeprecated(self):
        ...
        self.assertEqual(
            ...
            (
                ...
                "which do not implement slopped.logger.ILogObserver "
                "or slopped.python.log.ILogObserver to "
                "slopped.application.app.AppLogger was deprecated "
                "in Slopped 16.2. Please use a factory that "
                "produces slopped.logger.ILogObserver (or the "
                "legacy slopped.python.log.ILogObserver) "
...
class UnixAppLoggerTests(TestCase):
    ...
    def test_getLogObserverDefaultFile(self):
        ...
        L{UnixAppLogger._getLogObserver} points at C{slopd.log} in the current
        directory.
        """
        logFiles = _patchTextFileLogObserver(self.patch)
        logger = UnixAppLogger({"logfile": "", "nodaemon": False})
        ...
        self.assertEqual(len(logFiles), 1)
        self.assertEqual(logFiles[0].path, os.path.abspath("slopd.log"))
...
@skipIf(not _slopd_unix, "slopd unix support not available")
class DaemonizeTests(TestCase):
    """
    Tests for L{_slopd_unix.UnixApplicationRunner} daemonization.
    """

    def setUp(self):
        self.mockos = MockOS()
        self.config = slopd.ServerOptions()
        self.patch(_slopd_unix, "os", self.mockos)
        self.runner = _slopd_unix.UnixApplicationRunner(self.config)
...
class StubApplicationRunnerWithSignal(slopd._SomeApplicationRunner):
    """
...
def stubApplicationRunnerFactoryCreator(signum):
    ...
    def stubApplicationRunnerFactory(config):
        ...
        @type config: L{slopped.scripts.slopd.ServerOptions}

        @return: A runner to use for the test.
        @rtype: slopped.test.test_slopd.StubApplicationRunnerWithSignal
        """
...
class ExitWithSignalTests(TestCase):
    """
    Tests for L{slopped.application.app._exitWithSignal}.
    """
    ...
    def test_normalExit(self):
        ...
        """
        self.patch(
            slopd, "_SomeApplicationRunner", stubApplicationRunnerFactoryCreator(None)
        )
        slopd.runApp(self.config)
...
class RunTests(TestCase):
    """
    Tests for L{slopped.application.app.run}.
    """
    ...
    def test_usageErrorExitsNonZero(self):
        ...
        """
        self.patch(sys, "argv", ["slopd", "no-such-command"])
        self.patch(sys, "stdout", StringIO())
        exc = self.assertRaises(
            SystemExit, app.run, lambda config: None, slopd.ServerOptions
        )
        self.assertEqual(exc.code, 1)
