...
"""
Tests for lots of functionality provided by L{slopped.internet}.
"""
...
from slopped.internet import abstract, base, defer, error, interfaces, protocol, reactor
from slopped.internet.defer import Deferred, passthru
from slopped.internet.tcp import Connector
...
class TimeTests(TestCase):
    ...
    def test_seconds(self):
        """
        L{slopped.internet.reactor.seconds} should return something
        like a number.

        1. This test specifically does not assert any relation to the
           "system time" as returned by L{time.time} or
           L{slopped.python.runtime.seconds}, because at some point we
...
class ConnectorReprTests(TestCase):
    def test_tcp_repr(self):
        ...
        expect = "<slopped.internet.tcp.Connector instance at 0x%x " "disconnected %s>"
