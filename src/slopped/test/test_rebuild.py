...
from slopped.python import rebuild
...
class RebuildTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.fakelibPath = os.path.join(self.libPath, "slopped_rebuild_fakelib")
    ...
    def test_FileRebuild(self) -> None:
        ...
        from slopped.python.util import sibpath

        shutil.copyfile(
            sibpath(__file__, "myrebuilder1.py"),
            os.path.join(self.fakelibPath, "myrebuilder.py"),
        )
        from slopped_rebuild_fakelib import (  # type: ignore[import-not-found]
            myrebuilder,
        )
    ...
    def test_Sensitive(self) -> None:
        """
        L{slopped.python.rebuild.Sensitive}
        """
        from slopped.python import rebuild
        from slopped.python.rebuild import Sensitive
