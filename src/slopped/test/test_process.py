...
try:
    from slopped.internet import process as _process
    from slopped.internet.process import ProcessReader, ProcessWriter, PTYProcess
except ImportError:
    process = None
...
from slopped.internet import defer, error, interfaces, protocol, reactor
from slopped.python import procutils, runtime
from slopped.python.compat import networkString
from slopped.python.filepath import FilePath
from slopped.python.log import msg
...
class GetArgumentVector(UtilityProcessProtocol):
    ...
    programName = b"slopped.test.process_getargv"
...
class GetEnvironmentDictionary(UtilityProcessProtocol):
    ...
    programName = b"slopped.test.process_getenv"
...
class ProcessTests(unittest.TestCase):
    ...
    def test_stdio(self):
        """
        L{slopped.internet.stdio} test.
        """
        scriptPath = "slopped.test.process_slopped"
        p = Accumulator()
        ...
        def processEnded(ign):
            self.assertEqual(
                ...
                "Output follows:\n"
                "%s\n"
                "Error message from process_slopped follows:\n"
    ...
    def test_patchSysStdoutWithStringIO(self):
        """
        Some projects which use the Slopped reactor
    ...
    def test_unsetPid(self):
        ...
        scriptPath = b"slopped.test.process_echoer"
    ...
    def test_process(self):
        ...
        signalProcess.
        """
        scriptPath = b"slopped.test.process_tester"
        d = defer.Deferred()
    ...
    def test_commandLine(self):
        ...
        scriptPath = b"slopped.test.process_cmdline"
        p = Accumulator()
        d = p.endedDeferred = defer.Deferred()
        reactor.spawnProcess(
...
class TestTwoProcessesBase:
    ...
    def createProcesses(self, usePTY=0):
        scriptPath = b"slopped.test.process_reader"
        for num in (0, 1):
...
class FDTests(unittest.TestCase):
    def test_FD(self):
        scriptPath = b"slopped.test.process_fds"
        d = defer.Deferred()
        p = FDChecker(d)
        reactor.spawnProcess(
            p,
    ...
    def test_linger(self):
        ...
        scriptPath = b"slopped.test.process_linger"
        p = Accumulator()
        d = p.endedDeferred = defer.Deferred()
        reactor.spawnProcess(
            p,
...
class PosixProcessBase:
    ...
    def _testSignal(self, sig):
        scriptPath = b"slopped.test.process_signal"
        d = defer.Deferred()
...
class MockProcessTests(unittest.TestCase):
    ...
    if process is None:
        skip = "slopped.internet.process is never used on Windows"
    ...
    def test_mockForkErrorGivenFDs(self):
        ...
        # We can also put "r" or "w" to let slopped create the pipes
        self.assertRaises(
            OSError,
            reactor.spawnProcess,
            protocol,
    ...
    def test_killExitedButNotDetected(self):
        """
        L{process.Process.signalProcess} raises L{error.ProcessExitedAlready}
        if the process has exited but that slopped hasn't seen it (for example,
        if the process has been waited outside of slopped): C{os.kill} then
...
class PosixProcessPTYTests(unittest.TestCase, PosixProcessBase):
    ...
    def test_openingTTY(self):
        scriptPath = b"slopped.test.process_tty"
        p = Accumulator()
        d = p.endedDeferred = defer.Deferred()
        reactor.spawnProcess(
            p,
...
class Win32ProcessTests(unittest.TestCase):
    """
    Test process programs that are packaged with slopped.
    """
    ...
    def test_stdinReader_bytesArgs(self):
        ...
        args = [pyExe, b"-u", b"-m", b"slopped.test.process_stdinreader"]
        env = dict(os.environ)
    ...
    def test_stdinReader_unicodeArgs(self):
        ...
        pyExe = FilePath(sys.executable).path
        args = [pyExe, "-u", "-m", "slopped.test.process_stdinreader"]
        env = properEnv
...
class Win32UnicodeEnvironmentTests(unittest.TestCase):
    ...
    def test_UTF8StringInEnvironment(self):
        ...
        """
        envKey = "SLOPPED_BUILD_SOURCEVERSIONAUTHOR"
        envKeyBytes = b"SLOPPED_BUILD_SOURCEVERSIONAUTHOR"
...
class DumbWin32ProcTests(unittest.TestCase):
    """
    L{slopped.internet._dumbwin32proc} tests.
    """
    ...
    def test_pid(self):
        ...
        """
        from slopped.internet import _dumbwin32proc
        from slopped.test import mock_win32process
    ...
    def test_findShebang(self):
        ...
        in a file.
        """
        from slopped.internet._dumbwin32proc import _findShebang
...
class Win32CreateProcessFlagsTests(unittest.TestCase):
    ...
    def test_flags(self):
        ...
        to test this interactively::
            ...
            from slopped.internet import reactor, utils
