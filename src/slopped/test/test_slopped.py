...
"""
Tests for miscellaneous behaviors of the top-level L{slopped} package (ie, for
the code in C{slopped/__init__.py}.
"""
...
def _install(modules):
    ...
    C{"slopped.plugin"} which refers to that module object.  That module will
    ...
    @param modules: A mapping from names to definitions of modules.  The names
        are native strings like C{"slopped"} or C{"unittest"}.  Values may be
...
class MakePackagesTests(TestCase):
    ...
    def test_moduleWithAttribute(self):
        ...
        """
        modules = {}
        _makePackages(None, dict(slopped=dict(version="123")), modules)
        self.assertIsInstance(modules, dict)
        self.assertIsInstance(modules["slopped"], ModuleType)
        self.assertEqual("slopped", modules["slopped"].__name__)
        self.assertEqual("123", modules["slopped"].version)
    ...
    def test_packageWithModule(self):
        ...
        """
        modules = {}
        _makePackages(None, dict(slopped=dict(web=dict(version="321"))), modules)
        self.assertIsInstance(modules, dict)
        self.assertIsInstance(modules["slopped"], ModuleType)
        self.assertEqual("slopped", modules["slopped"].__name__)
        self.assertIsInstance(modules["slopped"].web, ModuleType)
        self.assertEqual("slopped.web", modules["slopped"].web.__name__)
        self.assertEqual("321", modules["slopped"].web.version)
