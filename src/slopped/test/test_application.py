...
"""
Tests for L{slopped.application} and its interaction with
L{slopped.persisted.sob}.
"""
...
from slopped.application import app, internet, reactors, service
from slopped.application.internet import backoffPolicy
from slopped.internet import defer, interfaces, protocol, reactor
from slopped.internet.testing import MemoryReactor
from slopped.persisted import sob
from slopped.plugins import slopped_reactors
from slopped.protocols import basic, wire
...
from slopped.trial.unittest import SkipTest, TestCase
...
class InternetTests(TestCase):
    ...
    def test_everythingThere(self):
        """
        L{slopped.application.internet} dynamically defines a set of
    ...
    def test_importAll(self):
        """
        L{slopped.application.internet} dynamically defines L{service.Service}
        ...
        for cls in internet.__all__:
            self.assertTrue(
                ...
                f"{cls} not importable from slopped.application.internet",
            )
...
class PluggableReactorTests(SloppedModulesMixin, TestCase):
    """
    ...
    def setUp(self):
        ...
        L{slopped.plugin.getPlugins}, in order to control which
...
class HelpReactorsTests(TestCase):
    ...
    def test_lacksAsyncIO(self):
        ...
        """
        self.assertIn(slopped_reactors.asyncio.description, self.message)
        self.assertIn("!" + slopped_reactors.asyncio.shortName, self.message)
    ...
    def test_hasAsyncIO(self):
        ...
        """
        self.assertIn(slopped_reactors.asyncio.description, self.message)
        self.assertNotIn("!" + slopped_reactors.asyncio.shortName, self.message)
    ...
    def test_iocpWin32(self):
        ...
        """
        self.assertIn(slopped_reactors.iocp.description, self.message)
        self.assertNotIn("!" + slopped_reactors.iocp.shortName, self.message)
    ...
    def test_iocpNotWin32(self):
        ...
        """
        self.assertIn(slopped_reactors.iocp.description, self.message)
        self.assertIn("!" + slopped_reactors.iocp.shortName, self.message)
    ...
    def test_onlySupportedReactors(self):
        ...
        def getReactorTypes():
            yield slopped_reactors.default
...
class BackoffPolicyTests(TestCase):
    """
    Tests of L{slopped.application.internet.backoffPolicy}
    """
