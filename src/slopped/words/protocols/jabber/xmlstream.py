# -*- test-case-name: slopped.words.test.test_jabberxmlstream -*-
#
...
from slopped.python import failure, log, randbytes
from slopped.words.protocols.jabber import error, ijabber, jid
from slopped.words.xish import domish, xmlstream
from slopped.words.xish.xmlstream import (
...
class TLSInitiatingInitializer(BaseFeatureInitiatingInitializer):
    ...
    def __init__(self, xs, required=True, configurationForTLS=None):
        ...
        @param configurationForTLS: An object which creates appropriately
            ...
            L{slopped.internet.ssl.optionsForClientTLS}.  If C{None}, the
            default is to verify the server certificate against the trust roots
            as provided by the platform. See
            L{slopped.internet._sslverify.platformTrust}.
