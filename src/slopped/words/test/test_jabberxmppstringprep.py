...
from slopped.words.protocols.jabber.xmpp_stringprep import (
    nameprep,
    nodeprep,
    resourceprep,
)
...
class DeprecationTests(unittest.TestCase):
    """
    Deprecations in L{slopped.words.protocols.jabber.xmpp_stringprep}.
    """
    ...
    def test_crippled(self) -> None:
        ...
        """
        from slopped.words.protocols.jabber.xmpp_stringprep import crippled

        warnings = self.flushWarnings(offendingFunctions=[self.test_crippled])
        self.assertEqual(DeprecationWarning, warnings[0]["category"])
        self.assertEqual(
            "slopped.words.protocols.jabber.xmpp_stringprep.crippled was "
            "deprecated in Slopped 13.1.0: crippled is always False",
            warnings[0]["message"],
        )
