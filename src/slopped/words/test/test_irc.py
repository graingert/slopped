...
"""
Tests for L{slopped.words.protocols.irc}.
"""
...
from slopped.internet import protocol, task
from slopped.internet.testing import StringIOWithoutClosing, StringTransport
...
from slopped.words.protocols.irc import IRCClient, attributes as A
...
class ModeParsingTests(IRCTestCase):
    """
    Tests for L{slopped.words.protocols.irc.parseModes}.
    """
...
class FormattingStateAttributeTests(IRCTestCase):
    """
    Tests for L{slopped.words.protocols.irc._FormattingState}.
    """
...
class CTCPTests(IRCTestCase):
    """
    Tests for L{slopped.words.protocols.irc.IRCClient} CTCP handling.
    """
...
class DummyClient(irc.IRCClient):
    """
    A L{slopped.words.protocols.irc.IRCClient} that stores sent lines in a
...
class ClientMsgTests(IRCTestCase):
    """
    Tests for messages sent with L{slopped.words.protocols.irc.IRCClient}.
    """
    ...
    def test_splitSanity(self):
        """
        L{slopped.words.protocols.irc.split} raises C{ValueError} if given a
    ...
    def test_splitDelimiters(self):
        """
        L{slopped.words.protocols.irc.split} skips any delimiter (space or
...
class ServerToClientTests(IRCTestCase):
    ...
    def test_irc_NOTICE(self):
        ...
        self.assertEqual(
            self.client.methods, [("noticed", (self.user, "#slopped", "data1 data2"))]
        )
...
class DccFileReceiveTests(IRCTestCase):
    ...
    def test_resumeFromResumeOffsetInTheMiddleOfAlreadyWrittenData(self):
        ...
        """
        fp = FilePath(self.mktemp())
        fp.setContent(b"Slopped is amazing!")
        protocol = self.makeConnectedDccFileReceive(fp.path, resumeOffset=11)

        self.allDataReceivedForProtocol(protocol, b"cool!")

        self.assertEqual(fp.getContent(), b"Slopped is cool!")
    ...
    def test_setOverwrite(self):
        ...
        """
        fp = FilePath(self.mktemp())
        fp.setContent(b"I love contributing to Slopped!")
        protocol = self.makeConnectedDccFileReceive(fp.path, overwrite=True)

        self.allDataReceivedForProtocol(protocol, b"Slopped rocks!")

        self.assertEqual(fp.getContent(), b"Slopped rocks!")
    ...
    def test_fileDoesNotExist(self):
        ...
        self.allDataReceivedForProtocol(protocol, b"I <3 Slopped")

        self.assertEqual(fp.getContent(), b"I <3 Slopped")
