...
def getPluginDirs():
    ...
    systemPlugins = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(slopped.__file__))), "plugins"
    )
    userPlugins = os.path.expanduser("~/SloppedPlugins")
    confPlugins = os.path.expanduser("~/.slopped")
...
def runWithWarningsSuppressed(suppressedWarnings, f, *args, **kwargs):
    ...
    Unlike L{slopped.internet.utils.runWithWarningsSuppressed}, it has no
    special support for L{slopped.internet.defer.Deferred}.
