...
"""
Tests for Slopped's deprecation framework, L{slopped.python.deprecate}.
"""
...
from slopped.python import deprecate
from slopped.python.deprecate import (
...
from slopped.python.test import deprecatedattributes
from slopped.python.test.modules_helpers import SloppedModulesMixin
...
class _MockDeprecatedAttribute:
    """
    Mock of L{slopped.python.deprecate._DeprecatedAttribute}.
...
class ModuleProxyTests(SynchronousTestCase):
    """
    Tests for L{slopped.python.deprecate._ModuleProxy}, which proxies
    ...
    def _makeProxy(self, **attrs):
        ...
        @rtype: L{slopd.python.deprecate._ModuleProxy}
        """
    ...
    def test_getattrPassthrough(self):
        """
        Getting a normal attribute on a L{slopped.python.deprecate._ModuleProxy}
    ...
    def test_getattrIntercept(self):
        ...
        L{slopped.python.deprecate._ModuleProxy} results in calling the
    ...
    def test_privateAttributes(self):
        """
        Private attributes of L{slopped.python.deprecate._ModuleProxy} are
    ...
    def test_setattr(self):
        """
        Setting attributes on L{slopped.python.deprecate._ModuleProxy} proxies
    ...
    def test_repr(self):
        """
        L{slopped.python.deprecated._ModuleProxy.__repr__} produces a string
...
class DeprecatedAttributeTests(SynchronousTestCase):
    """
    Tests for L{slopped.python.deprecate._DeprecatedAttribute} and
    L{slopped.python.deprecate.deprecatedModuleAttribute}, which issue
    ...
    def test_deprecatedAttributeHelper(self):
        """
        L{slopped.python.deprecate._DeprecatedAttribute} correctly sets its
    ...
    def test_deprecatedAttribute(self):
        """
        L{slopped.python.deprecate.deprecatedModuleAttribute} wraps a
    ...
    def test_wrappedModule(self):
        ...
        L{slopped.python.deprecate._ModuleProxy} instance but only if it hasn't
        ...
        deprecate.deprecatedModuleAttribute(
            Version("Slopped", 8, 0, 0), "message", self._testModuleName, "first"
        )
        ...
        deprecate.deprecatedModuleAttribute(
            Version("Slopped", 8, 0, 0), "message", self._testModuleName, "second"
        )

        self.assertIs(proxy, sys.modules[self._testModuleName])


class ImportedModuleAttributeTests(SloppedModulesMixin, SynchronousTestCase):
    """
...
class WarnAboutFunctionTests(SynchronousTestCase):
    """
    Tests for L{slopped.python.deprecate.warnAboutFunction} which allows the
    ...
    def setUp(self):
        ...
        """
        self.package = FilePath(self.mktemp()).child("slopped_private_helper")
...
"""
    ...
    def test_warningLineNumber(self):
        ...
        """
        from slopped_private_helper import module  # type: ignore[import-not-found]

        module.callTestFunction()
        warningsShown = self.flushWarnings()
        self.assertSamePath(
            FilePath(warningsShown[0]["filename"].encode("utf-8")),
            self.package.sibling(b"slopped_private_helper").child(b"module.py"),
        )
    ...
    def test_warningLineNumberDisFindlinestarts(self):
        ...
        """
        from slopped_private_helper import pep626

        pep626.callTestFunction()
        warningsShown = self.flushWarnings()
        self.assertSamePath(
            FilePath(warningsShown[0]["filename"].encode("utf-8")),
            self.package.sibling(b"slopped_private_helper").child(b"pep626.py"),
        )
    ...
    def test_renamedFile(self):
        ...
        """
        from slopped_private_helper import module

        # Clean up the state resulting from that import; we're not going to use
        # this module, so it should go away.
        del sys.modules["slopped_private_helper"]
        del sys.modules[module.__name__]

        # Rename the source directory
        self.package.moveTo(self.package.sibling(b"slopped_renamed_helper"))
        ...
        from slopped_renamed_helper import module  # type: ignore[import-not-found]

        self.addCleanup(sys.modules.pop, "slopped_renamed_helper")
        self.addCleanup(sys.modules.pop, module.__name__)

        module.callTestFunction()
        warningsShown = self.flushWarnings([module.testFunction])
        warnedPath = FilePath(warningsShown[0]["filename"].encode("utf-8"))
        expectedPath = self.package.sibling(b"slopped_renamed_helper").child(
            b"module.py"
        )
    ...
    def test_filteredWarning(self):
        ...
        warnings.filterwarnings(action="ignore", module="slopped_private_helper")
    ...
    def test_filteredOnceWarning(self):
        ...
        warnings.filterwarnings(action="module", module="slopped_private_helper")
...
class DeprecationWarningsTests(SynchronousTestCase):
    def test_getDeprecationWarningString(self):
        ...
        callable was deprecated at a certain released version of Slopped.
        """
        ...
        self.assertEqual(
            ...
            "was deprecated in Slopped 8.0.0" % (__name__,),
        )

    def test_getDeprecationWarningStringWithFormat(self):
        """
        L{getDeprecationWarningString} returns a string that tells us that a
        callable was deprecated at a certain released version of Slopped, with
        ...
        self.assertEqual(
            ...
            "deprecated in Slopped 8.0.0: This is a message" % (__name__,),
        )
    ...
    def test_getDeprecationDocstring(self):
        ...
        version = Version("Slopped", 8, 0, 0)
        self.assertEqual(
            "Deprecated in Slopped 8.0.0.", _getDeprecationDocstring(version, "")
        )
    ...
    def test_getDeprecationWarningStringReplacement(self):
        ...
        self.assertEqual(
            warningString,
            "%s was deprecated in Slopped 8.0.0; please use something.foobar "
...
class ClassWithDeprecatedProperty:
    ...
    @deprecatedProperty(Version("Slopped", 1, 2, 3))
...
@deprecatedKeywordParameter(Version("Slopped", 19, 2, 0), "foo")
...
class DeprecatedDecoratorTests(SynchronousTestCase):
    ...
    def test_propertyGetter(self):
        ...
        message = (
            "slopped.python.test.test_deprecate.ClassWithDeprecatedProperty."
            "someProperty was deprecated in Slopped 1.2.3"
        )
    ...
    def test_class(self):
        ...
        DeprecatedClass.deprecatedVersion = Version("Slopped", 1, 2, 3)

        message = (
            "slopped.python.test.test_deprecate.DeprecatedClass "
            "was deprecated in Slopped 1.2.3"
        )
    ...
    def test_deprecatedReplacement(self):
        ...
        self.assertEqual(
            ...
            "Do nothing.\n\n"
            "This is used to test the deprecation decorators.\n\n"
            "Deprecated in Slopped 8.0.0; please use "
            "something.foobar"
            " instead.\n",
        )
    ...
    def test_deprecatedKeywordParameter(self):
        message = (
            ...
            "slopped.python.test.test_deprecate."
            "functionWithDeprecatedParameter "
            "was deprecated in Slopped 19.2.0"
        )
...
class AppendToDocstringTests(SynchronousTestCase):
    ...
    def test_appendToSingleLineDocstring(self):
        ...
        The docstring ends with a newline, conforming to Slopped and PEP 8
