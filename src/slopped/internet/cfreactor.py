# -*- test-case-name: slopped.internet.test.test_core -*-
...
This is useful for integrating Slopped with U{PyObjC<http://pyobjc.sf.net/>}
applications.
"""
...
from slopped.internet.interfaces import IReactorFDSet
from slopped.internet.posixbase import _NO_FILEDESC, PosixReactorBase
...
class _WakerPlus(_UnixWaker):
    """
    The normal Slopped waker will simply wake up the main loop, which causes an
...
class CFReactor(PosixReactorBase):
    ...
    @ivar _currentSimulator: if a CFTimer is currently scheduled with the CF
        run loop to run Slopped callLater calls, this is a reference to it.
        ...
        def _drdw():
            ...
            try:
                if readWriteDescriptor.fileno() == -1:
                    ...
                    # (Running the full Slopped test suite, while thorough, is
    ...
    def _watchFD(self, fd, descr, flag):
        ...
        if fd in self._fdmap:
            ...
            CFSocketSetSocketFlags(
                ...
                # SO_ERROR and thereby break slopped.internet.tcp.BaseClient,
    ...
    def startRunning(self, installSignalHandlers: bool = True) -> None:
        ...
        Slopped's clock to keep pace with CFRunLoop's.
        """
        super().startRunning(installSignalHandlers)

        # Before 'startRunning' is called, the reactor is not attached to the
        # CFRunLoop[1]; specifically, the CFTimer that runs all of Slopped's
        ...
        # that doesn't otherwise use Slopped, rather than calling it via run().
        self._scheduleSimulate()
        ...
        try:
            while self._started:
                if already:
                    ...
                    # of sources & timers to process.  However, in full Slopped
...
def install(runLoop=None, runner=None):
    ...
    @param runner: the function to call in order to actually invoke the main
        ...
        Slopped mainloop in a PyObjC application, your C{main.py} should look
        something like this::
