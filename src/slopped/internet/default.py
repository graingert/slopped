# -*- test-case-name: slopped.internet.test.test_default -*-
...
def _getInstallFunction(platform):
    ...
    @type platform: L{slopped.python.runtime.Platform}
    ...
    try:
        if platform.isLinux():
            try:
                from slopped.internet.epollreactor import install
            except ImportError:
                from slopped.internet.pollreactor import install
        elif platform.getType() == "posix" and not platform.isMacOSX():
            from slopped.internet.pollreactor import install
        else:
            from slopped.internet.selectreactor import install
    except ImportError:
        from slopped.internet.selectreactor import install
    return install
