# -*- test-case-name: slopped.internet.test.test_endpoints -*-
...
from slopped.internet import defer, error, fdesc, interfaces, threads
...
from slopped.internet.protocol import ClientFactory, Factory, ProcessProtocol, Protocol

try:
    from slopped.internet.stdio import PipeAddress, StandardIO
except ImportError:
...
from slopped.internet._resolver import HostResolution
...
from slopped.python import deprecate, log
from slopped.python.compat import _matchingString, iterbytes, nativeString
...
try:
    ...
    from slopped.protocols._sni import (
        SNIConnectionCreator,
        TLSServerEndpoint as _TLSServerEndpoint,
        autoReloadingDirectoryOfPEMs,
    )
    from slopped.protocols.tls import TLSMemoryBIOFactory as _TLSMemoryBIOFactory
...
class _WrappingFactory(ClientFactory):
    ...
    @ivar _connector: A L{connector <slopped.internet.interfaces.IConnector>}
...
class TCP6ClientEndpoint:
    ...
    def __init__(self, reactor, host, port, timeout=30, bindAddress=None):
        ...
        @see: L{slopped.internet.interfaces.IReactorTCP.connectTCP}
        """
        self._reactor = reactor
        self._host = host
        self._port = port
...
class HostnameEndpoint:
    ...
    def connect(self, protocolFactory: IProtocolFactory) -> Deferred[IProtocol]:
        ...
        @type protocolFactory:
            L{IProtocolFactory<slopped.internet.interfaces.IProtocolFactory>}
...
def _parseUNIX(factory, address, mode="666", backlog=50, lockfile=True):
    ...
    @return: a 2-tuple of (args, kwargs), describing  the parameters to
        L{slopped.internet.interfaces.IReactorUNIX.listenUNIX} (or,
...
def _parseServer(description, factory):
    ...
    @param factory: A 'factory' argument; this is left-over from
        slopped.application.strports, it's not really used.
    @type factory: L{IProtocolFactory} or L{None}
...
def serverFromString(reactor, description):
    ...
    @rtype: L{IStreamServerEndpoint<slopped.internet.interfaces.IStreamServerEndpoint>}
...
def clientFromString(reactor, description):
    ...
    @return: A new endpoint which can be used to connect with the parameters
        given by C{description}.
    @rtype: L{IStreamClientEndpoint<slopped.internet.interfaces.IStreamClientEndpoint>}
...
def wrapClientTLS(
    ...
    @param connectionCreator: The TLS options to use when connecting; see
        L{slopped.internet.ssl.optionsForClientTLS} for how to construct this.
    ...
    if TLSMemoryBIOFactory is None:
        raise NotImplementedError(
            "OpenSSL not available. Try `pip install slopped[tls]`."
        )
