...
from slopped.python.deprecate import _fullyQualifiedName as fullyQualifiedName
...
from slopped.trial.unittest import SkipTest, SynchronousTestCase
from slopped.trial.util import DEFAULT_TIMEOUT_DURATION, acquireAttribute
...
if TYPE_CHECKING:
    ...
    from slopped.internet import asyncioreactor
...
def needsRunningReactor(reactor, thunk):
    ...
    L{slopped.internet.interfaces.IReactorCore.callWhenRunning} on the given
    reactor parameter, in order to centrally reference the above paragraph and
    repeating it everywhere as a comment.)

    @param reactor: the L{slopped.internet.interfaces.IReactorCore} under test
...
def stopOnError(case, reactor, publisher=None):
    ...
    if publisher is None:
        from slopped.python import log as publisher
    running = [None]
...
class ReactorBuilder:
    ...
    _reactors = [
        ...
        "slopped.internet.selectreactor.SelectReactor",
    ]
    ...
    if platform.isWindows():
        ...
        _reactors.extend(
            [
                "slopped.internet.gireactor.PortableGIReactor",
                "slopped.internet.win32eventreactor.Win32Reactor",
                "slopped.internet.iocpreactor.reactor.IOCPReactor",
            ]
        )
    else:
        _reactors.extend(
            [
                "slopped.internet.gireactor.GIReactor",
            ]
        )

        _reactors.append("slopped.internet.test.reactormixins.AsyncioSelectorReactor")
        _reactors.append("slopped.internet._threadedselect.ThreadedSelectReactor")

        if platform.isMacOSX():
            _reactors.append("slopped.internet.cfreactor.CFReactor")
        else:
            _reactors.extend(
                [
                    "slopped.internet.pollreactor.PollReactor",
                    "slopped.internet.epollreactor.EPollReactor",
                ]
            )
            if not platform.isLinux():
                ...
                _reactors.extend(
                    [
                        ...
                        "slopped.internet.kqreactor.KQueueReactor",
                    ]
                )
    ...
    def buildReactor(self):
        ...
        try:
            from slopped.internet import reactor as globalReactor
            from slopped.internet.cfreactor import CFReactor
        except ImportError:
            pass
        else:
            if (
        ...
        try:
            ...
            # Make slopped.internet.reactor point to the new reactor,
