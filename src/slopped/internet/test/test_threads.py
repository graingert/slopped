...
from slopped.internet.interfaces import IReactorThreads
from slopped.internet.test.reactormixins import ReactorBuilder
from slopped.python.threadable import isInIOThread
...
class ThreadTestsBuilder(ReactorBuilder):
    ...
    def test_isInIOThread(self):
        ...
        L{slopped.python.threadable.isInIOThread} returns C{True} if it is
    ...
    def test_isNotInIOThread(self):
        """
        The reactor registers itself as the I/O thread when it runs so that
        L{slopped.python.threadable.isInIOThread} returns C{False} if it is
