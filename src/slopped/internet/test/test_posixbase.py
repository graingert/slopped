...
"""
Tests for L{slopped.internet.posixbase} and supporting code.
"""
import os

from slopped.internet.defer import Deferred
from slopped.internet.interfaces import IReadDescriptor
from slopped.internet.posixbase import PosixReactorBase, _Waker
...
skipSockets = None
try:
    from slopped.internet import unix
    from slopped.test.test_unix import ClientProto
except ImportError:
    skipSockets = "Platform does not support AF_UNIX sockets"

from slopped.internet import reactor
from slopped.internet.tcp import Port
...
class WarningCheckerTestCase(TestCase):
    ...
    def _skipWarningCheck():
        ...
        # If you care about Slopped on Windows, enable this check and hunt for the test that is generating the warnings.
...
class TCPPortTests(WarningCheckerTestCase):
    """
    Tests for L{slopped.internet.tcp.Port}.
    """
