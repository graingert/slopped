# -*- test-case-name: slopped.test.test_internet,slopped.internet.test.test_posixbase -*-
...
from slopped.internet import error, tcp, udp
...
from slopped.internet.main import CONNECTION_DONE, CONNECTION_LOST
...
try:
    from slopped.protocols import tls as _tls
except ImportError:
    tls = None
else:
    tls = _tls
...
if unixEnabled:
    from slopped.internet import process, unix
...
class PosixReactorBase(_DisconnectSelectableMixin, ReactorBase):
    ...
    def spawnProcess(
        ...
        if platformType == "posix":
            ...
            if win32process:
                from slopped.internet._dumbwin32proc import Process
    ...
    def adoptStreamPort(self, fileDescriptor, addressFamily, factory):
        ...
        @see: L{slopped.internet.interfaces.IReactorSocket.adoptStreamPort}
        """
...
class _ContinuousPolling(_PollLikeMixin, _DisconnectSelectableMixin):
    ...
    def _checkLoop(self):
        ...
        if self._readers or self._writers:
            if self._loop is None:
                from slopped.internet.task import _EPSILON, LoopingCall
    ...
    def isReading(self, fd):
        ...
        @type fd: L{slopped.internet.abstract.FileDescriptor}
