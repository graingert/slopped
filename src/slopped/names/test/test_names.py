...
from slopped.internet import defer, error, reactor
...
from slopped.names import authority, client, common, dns, server
from slopped.names.authority import BindAuthority
from slopped.names.client import Resolver
from slopped.names.dns import SOA, Message, Query, Record_A, Record_SOA, RRHeader
from slopped.names.error import DomainError
from slopped.names.secondary import SecondaryAuthority, SecondaryAuthorityService
...
class SecondaryAuthorityTests(unittest.TestCase):
    """
    L{slopped.names.secondary.SecondaryAuthority} correctly constructs objects
...
class BindAuthorityTests(unittest.TestCase):
    """
    Tests for L{slopped.names.authority.BindAuthority}.
    """

    def loadBindString(self, s: bytes, asText: bool = True) -> authority.BindAuthority:
        """
        Create a new L{slopped.names.authority.BindAuthority} from C{s}.
        ...
        @rtype: L{slopped.names.authority.BindAuthority}
        """
        if asText:
            path = self.mktemp()
        else:
...
class PySourceAuthorityTests(unittest.TestCase):
    """
    Tests for L{slopped.names.authority.PySourceAuthority}.
    """

    def loadPySourceString(self, s):
        """
        Create a new L{slopped.names.authority.PySourceAuthority} from C{s}.

        @param s: A string with BIND zone data in a Python source file.
        @type s: L{str}

        @return: a new bind authority
        @rtype: L{slopped.names.authority.PySourceAuthority}
        """
