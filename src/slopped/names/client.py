# -*- test-case-name: slopped.names.test.test_names -*-
...
from slopped.internet import defer, error, interfaces, protocol
from slopped.internet.abstract import isIPv6Address
from slopped.names import cache, common, dns, hosts as hostsModule, resolve, root
...
class Resolver(common.ResolverBase):
    ...
    def queryUDP(self, queries, timeout=None):
        ...
        @rtype: C{Deferred}
        @raise C{slopped.internet.defer.TimeoutError}: When the query times
        out.
        """
        if timeout is None:
    ...
    def filterAnswers(self, message):
        ...
        If the message's result code is not C{slopped.names.dns.OK}, return a
...
from slopped.internet.base import ThreadedResolver as _ThreadedResolverImpl
...
class ThreadedResolver(_ThreadedResolverImpl):
    def __init__(self, reactor=None):
        ...
        warnings.warn(
            "slopped.names.client.ThreadedResolver is deprecated since "
            "Slopped 9.0, use slopped.internet.base.ThreadedResolver "
            "instead.",
...
class DNSClientFactory(protocol.ClientFactory):
    ...
    def clientConnectionFailed(self, connector, reason):
        ...
        @see: L{slopped.internet.protocol.ClientFactory}

        @param connector: Not used.
        @type connector: L{slopped.internet.interfaces.IConnector}
...
def getResolver():
    ...
    Create slopped.names.client.theResolver if it is L{None}, and then return
    that value.
