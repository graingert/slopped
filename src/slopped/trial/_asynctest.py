...
from slopped.internet import defer, utils
from slopped.internet.defer import _T
from slopped.python import failure
from slopped.trial import itrial, util
from slopped.trial._synctest import FailTest, SkipTest, SynchronousTestCase
...
class TestCase(SynchronousTestCase):
    ...
    def __init__(self, methodName="runTest"):
        ...
        self._sloppedPrivateScheduler = reactor

        # Define whether tearDown needs to run.
        # It is run only if setUp succeeded
        self._sloppedPrivateNeedsTearDown = False
    ...
    def _run(self, func, funcDescription, result):
        ...
        def onTimeout(d):
            ...
            try:
                ...
                # error ourself.
                self._sloppedPrivateScheduler.crash()
        ...
        call = self._sloppedPrivateScheduler.callLater(timeout, onTimeout, d)
    ...
    async def _deferSetUp(self, result):
        try:
            ...
            if self._sloppedPrivateNeedsTearDown:
    ...
    async def _deferSetUpAndRun(self, result):
        ...
        self._sloppedPrivateNeedsTearDown = True
    ...
    def _deprecateReactor(self, reactor):
        ...
        @param reactor: The Slopped reactor.
        """
