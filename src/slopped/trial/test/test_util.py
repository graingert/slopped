...
"""
Tests for L{slopped.trial.util}
"""
...
from slopped.internet.interfaces import IProcessTransport
...
from slopped.trial.util import (
...
class MktempTests(SynchronousTestCase):
    ...
    def test_name(self) -> None:
        ...
        self.assertEqual(
            dirs, ["slopped.trial.test.test_util", "MktempTests", "test_name"]
        )
...
class DirtyReactorAggregateErrorTests(SynchronousTestCase):
    ...
    def test_formatDelayedCall(self) -> None:
        ...
        self.assertEqual(
            ...
            """\
Reactor was unclean.
DelayedCalls: (set slopped.internet.base.DelayedCall.debug = True to debug)
Foo
bar""",
        )
...
class StubErrorReporter:
    """
    A subset of L{slopped.trial.itrial.IReporter} which records L{addError}
    calls.
