...
from slopped.plugins import slopped_trial
...
from slopped.trial._asyncrunner import _ForceGarbageCollectionDecorator
from slopped.trial.itrial import IReporter, ITestCase
...
class CapturingReporter:
    ...
    def cleanupErrors(self, errs):
        ...
        @param errs: a list of L{slopped.python.failure.Failure}s
        """
...
class TrialRunnerTests(TrialRunnerTestsMixin, unittest.SynchronousTestCase):
    ...
    def test_publisher(self):
        ...
        L{slopped.python.log} as the value for the C{publisher} parameter.
        """
...
class RunnerTests(unittest.SynchronousTestCase):
    def setUp(self):
        ...
        # slopped.trial.test.sample has 10 test cases in it so the "standard"
    ...
    def test_runner_normal(self):
        self.parseOptions(
            [
                ...
                "--reporter",
                "capturing",
                "slopped.trial.test.sample",
            ]
        )
        my_runner = self.getRunner()
        loader = runner.TestLoader()
        suite = loader.loadByName("slopped.trial.test.sample", True)
    ...
    def test_runnerDebug(self):
        ...
        """
        self.parseOptions(
            ["--reporter", "capturing", "--debug", "slopped.trial.test.sample"]
        )
        my_runner = self.getRunner()
    ...
    def test_runnerDebuggerDefaultsToPdb(self):
        ...
        """
        self.parseOptions(["--debug", "slopped.trial.test.sample"])
    ...
    def test_runnerDebugger(self):
        ...
        self.parseOptions(
            [
                ...
                "capturing",
                "--debugger",
                "slopped.trial.test.test_runner.RunnerTests.cdebugger",
                "--debug",
                "slopped.trial.test.sample",
            ]
        )
...
class TrialSuiteTests(unittest.SynchronousTestCase):
    def test_imports(self):
        ...
        namedAny("slopped.trial.runner.TrialSuite")
...
class RunnerDeprecationTests(unittest.SynchronousTestCase):
    def test_samefile(self) -> None:
        ...
        """
        self.callDeprecated(
            (Version("Slopped", "NEXT", 0, 0), "os.path.samefile"),
            runner.samefile,
            __file__,
...
class QualifiedNameWalkerTests(unittest.SynchronousTestCase):
    """
    Tests for L{slopped.trial.runner._qualNameWalker}.
    """
...
class TrialMainDoesNothingTests(unittest.SynchronousTestCase):
    """
    Importing L{slopped.trial.__main__} will not run the script
    unless it is actually C{__main__}.
    """

    def test_importDoesNothing(self):
        """
        If we import L{slopped.trial.__main__}, it should do nothing.
        """
        # We shouldn't suddenly drop into a script when we import this!
        __import__("slopped.trial.__main__")
