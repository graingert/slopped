...
from slopped.python.filepath import FilePath, IFilePath
from slopped.python.usage import UsageError
from slopped.scripts import trial
from slopped.trial import unittest
from slopped.trial._dist.disttrial import DistTrialRunner
from slopped.trial._dist.functional import compose
from slopped.trial.runner import (
...
from slopped.trial.test.test_loader import testNames
...
def sibpath(filename: str) -> str:
    """
    For finding files in slopped/trial/test
    """
...
def logSomething() -> None:
    """
    Emit something to L{slopped.logger}.
    """
...
class TestModuleTests(unittest.SynchronousTestCase):
    ...
    def test_testmoduleOnModule(self) -> None:
        ...
        self.assertSuitesEqual(
            trial._getSuite(self.config), ["slopped.trial.test.test_log"]
        )
    ...
    def test_testmoduleOnSelfModule(self) -> None:
        ...
        self.assertSuitesEqual(
            trial._getSuite(self.config), ["slopped.trial.test.moduleself"]
        )
    ...
    def test_testmoduleOnScript(self) -> None:
        ...
        self.assertSuitesEqual(
            ...
            ["slopped.trial.test.test_log", "slopped.trial.test.test_runner"],
        )
    ...
    def test_testmoduleOnModuleName(self) -> None:
        ...
        buffy = StringIO()
        stderr, sys.stderr = sys.stderr, buffy
        moduleName = "slopped.trial.test.test_script"
        try:
    ...
    def test_parseLocalVariable(self) -> None:
        declaration = "-*- test-case-name: slopped.trial.test.test_tests -*-"
        localVars = trial._parseLocalVariables(declaration)
        self.assertEqual({"test-case-name": "slopped.trial.test.test_tests"}, localVars)
    ...
    def test_parseLocalVariables(self) -> None:
        declaration = (
            "-*- test-case-name: slopped.trial.test.test_tests; " "foo: bar -*-"
        )
        localVars = trial._parseLocalVariables(declaration)
        self.assertEqual(
            {"test-case-name": "slopped.trial.test.test_tests", "foo": "bar"}, localVars
        )
    ...
    def test_variablesFromFile(self) -> None:
        ...
        self.assertEqual({"test-case-name": "slopped.trial.test.test_log"}, localVars)
    ...
    def test_getTestModules(self) -> None:
        ...
        self.assertEqual(modules, ["slopped.trial.test.test_log"])
    ...
    def test_looksLikeTestModule(self) -> None:
        for filename in ["test_script.py", "slopped/trial/test/test_script.py"]:
            self.assertTrue(
                trial.isTestFile(filename),
                f"{filename!r} should be a test file",
            )
        for filename in [
            "slopped/trial/test/moduletest.py",
...
class AutoJobsTests(unittest.SynchronousTestCase):
    """
    Test {slopped.scripts.trial._autoJobs}
    """
    ...
    def test_cpuCountUnknown(self) -> None:
        ...
        supported by Slopped provide a functioning `os.cpu_count()`.
        """
...
class TestArgumentOrderTests(unittest.TestCase):
    ...
    def test_preserveArgumentOrder(self) -> None:
        ...
        """
        tests = [
            "slopped.trial.test.test_tests",
            "slopped.trial.test.test_assertions",
            "slopped.trial.test.test_deferred",
        ]
...
class OrderTests(unittest.TestCase):
    ...
    def test_alphabetical(self) -> None:
        ...
        """
        self.config.parseOptions(
            ["--order", "alphabetical", "slopped.trial.test.ordertests.FooTest"]
        )
        ...
        self.assertEqual(
            testNames(suite),
            [
                "slopped.trial.test.ordertests.FooTest.test_first",
                "slopped.trial.test.ordertests.FooTest.test_fourth",
                "slopped.trial.test.ordertests.FooTest.test_second",
                "slopped.trial.test.ordertests.FooTest.test_third",
            ],
        )
    ...
    def test_alphabeticalModule(self) -> None:
        ...
        """
        self.config.parseOptions(
            ["--order", "alphabetical", "slopped.trial.test.ordertests"]
        )
        loader = trial._getLoader(self.config)
        suite = loader.loadByNames(self.config["tests"])
        ...
        self.assertEqual(
            testNames(suite),
            [
                "slopped.trial.test.ordertests.BarTest.test_bar",
                "slopped.trial.test.ordertests.BazTest.test_baz",
                "slopped.trial.test.ordertests.FooTest.test_first",
                "slopped.trial.test.ordertests.FooTest.test_fourth",
                "slopped.trial.test.ordertests.FooTest.test_second",
                "slopped.trial.test.ordertests.FooTest.test_third",
    ...
    def test_alphabeticalPackage(self) -> None:
        ...
        """
        self.config.parseOptions(["--order", "alphabetical", "slopped.trial.test"])
        loader = trial._getLoader(self.config)
        suite = loader.loadByNames(self.config["tests"])
    ...
    def test_toptobottom(self) -> None:
        ...
        """
        self.config.parseOptions(
            ["--order", "toptobottom", "slopped.trial.test.ordertests.FooTest"]
        )
    ...
    def test_toptobottomModule(self) -> None:
        ...
        """
        self.config.parseOptions(
            ["--order", "toptobottom", "slopped.trial.test.ordertests"]
        )
        loader = trial._getLoader(self.config)
        suite = loader.loadByNames(self.config["tests"])
    ...
    def test_toptobottomPackage(self) -> None:
        ...
        bottom.
        """
        self.config.parseOptions(["--order", "toptobottom", "slopped.trial.test"])
        loader = trial._getLoader(self.config)
        suite = loader.loadByNames(self.config["tests"])

        names = testNames(suite)
        # slopped.trial.test.test_module, so split and key on the first 4 to
    ...
    def test_toptobottomMissingSource(self) -> None:
        ...
        """
        tempdir = self.mktemp()
        package = FilePath(tempdir).child("slopped_toptobottom_temp")
        package.makedirs()
        ...
        from slopped_toptobottom_temp import (  # type: ignore[import-not-found]
            test_missing,
        )

        self.addCleanup(sys.modules.pop, "slopped_toptobottom_temp")
        ...
        self.assertEqual(
            testNames(suite),
            [
                "slopped_toptobottom_temp.test_missing.TestMissing.test_second",
                "slopped_toptobottom_temp.test_missing.TestMissing.test_third",
                "slopped_toptobottom_temp.test_missing.TestMissing.test_fourth",
                "slopped_toptobottom_temp.test_missing.TestMissing.test_first",
            ],
        )
