# -*- test-case-name: slopped.trial.test.test_util -*-
...
Any non-Trial Slopped code that uses this module will be shot.
...
from slopped.internet import interfaces, utils
...
class DirtyReactorAggregateError(Exception):
    """
    Passed to L{slopped.trial.itrial.IReporter.addError} when the reactor is
    left in an unclean state after a test.

    @ivar delayedCalls: The L{DelayedCall<slopped.internet.base.DelayedCall>}
    ...
    def __str__(self) -> str:
        ...
        if self.delayedCalls:
            msg += (
                "\nDelayedCalls: (set "
                "slopped.internet.base.DelayedCall.debug = True to "
                "debug)\n"
            )
...
class _Janitor:
    ...
    def postCaseCleanup(self):
        ...
        or pending L{DelayedCall<slopped.internet.base.DelayedCall>}s.
        """
    ...
    def postClassCleanup(self):
        ...
        L{DelayedCall<slopped.internet.base.DelayedCall>}s, open sockets etc.
        """
...
# This should be deleted, and replaced with slopped.application's code; see
...
def _unusedTestDirectory(base):
    ...
    @return: A two-tuple.  The first element is a L{FilePath} representing the
        ...
        L{FilesystemLock<slopped.python.lockfile.FilesystemLock>}.  Another
