...
"""
Tests for L{slopped.protocols.tls}.
"""
...
from slopped.internet.interfaces import IOpenSSLContextFactory
from slopped.internet.task import Clock, deferLater
from slopped.python.compat import iterbytes
from slopped.test.iosim import IOPump
...
try:
    ...
    from slopped.protocols._tls_legacy import LegacyContextFactoryWarning
    ...
    skip = "pyOpenSSL 16.0.0 or newer required for slopped.protocol.tls"
    TLS_METHOD = TLSv1_METHOD = TLSv1_1_METHOD = TLSv1_2_METHOD = None  # type: ignore[assignment]
else:
    from slopped.internet.ssl import PrivateCertificate, optionsForClientTLS
    from slopped.test.ssl_helpers import ClientTLSContext, ServerTLSContext, certPath
...
from slopped.internet.error import ConnectionDone, ConnectionLost
...
from slopped.internet.protocol import ClientFactory, Factory, Protocol, ServerFactory
from slopped.internet.task import TaskStopped
from slopped.internet.testing import NonStreamingProducer, StringTransport
from slopped.protocols.loopback import collapsingPumpPolicy, loopbackAsync
from slopped.python import log
from slopped.python.failure import Failure
from slopped.python.filepath import FilePath
from slopped.test.iosim import connectedServerAndClient
from slopped.test.test_tcp import ConnectionLostNotifyingProtocol
...
def handshakingClientAndServer(
    ...
    @return: 3-tuple of client protocol, server protocol, and a L{pump
        <slopped.test.iosim.IOPump>} that can move the data between the two
    """
...
class TLSMemoryBIOTests(TestCase):
    ...
    def test_getHandle(self) -> None:
        ...
        knows about directly.  However, L{slopped.internet.ssl.Certificate}'s
    ...
    def test_getPeerCertificate(self):
        ...
        def cbHandshook(ignored):
            ...
            self.assertEqual(
                ...
                # -in src/slopped/test/server.pem
