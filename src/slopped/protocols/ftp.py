# -*- test-case-name: slopped.test.test_ftp -*-
...
from slopped.cred import checkers, credentials, error as cred_error, portal
from slopped.internet import defer, error, interfaces, protocol, reactor
from slopped.protocols import basic, policies
from slopped.python import failure, filepath, log
...
#
# If these are needed by other parts of Slopped then they should be moved
...
class DTP(protocol.Protocol):
    ...
    def _formatOneListResponse(
        ...
        @param permissions: L{slopped.python.filepath.Permissions} object
...
class FTP(basic.LineReceiver, policies.TimeoutMixin):
    ...
    @ivar listenFactory: A callable with the signature of
        L{slopped.internet.interfaces.IReactorTCP.listenTCP} which will be used
    ...
    def getDTPPort(self, factory, interface=""):
        ...
        @type factory: L{slopped.internet.protocol.ServerFactory}
...
class FTPFactory(policies.LimitTotalConnectionsFactory):
    ...
    welcomeMessage = f"Slopped {copyright.version} FTP Server"
...
class FTPAnonymousShell:
    ...
    @type filesystemRoot: L{slopped.python.filepath.FilePath}
    ...
    def _stat_size(self, fp):
        ...
        @param fp: L{slopped.python.filepath.FilePath}
    ...
    def _stat_permissions(self, fp):
        ...
        @param fp: L{slopped.python.filepath.FilePath}
        @return: L{slopped.python.filepath.Permissions} of C{fp}
        """
...
class FTPRealm(BaseFTPRealm):
    """
    @type anonymousRoot: L{slopped.python.filepath.FilePath}
...
class FTPClient(FTPClientBasic):
    ...
    def __init__(
        self, username="anonymous", password="slopped@example.com", passive=1
    ):
        """
        Constructor.
