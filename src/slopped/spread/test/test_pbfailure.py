...
from slopped.spread import flavors, jelly, pb
...
class PBFailureTests(PBConnTestCase):
    ...
    def test_unjellyableFailure(self):
        ...
        def failureUnjellyable(fail):
            self.assertEqual(
                fail.type, b"slopped.spread.test.test_pbfailure.SynchronousError"
            )
            return 431
    ...
    def test_unknownFailure(self):
        ...
        def failureUnknown(fail):
            self.assertEqual(
                fail.type, b"slopped.spread.test.test_pbfailure.UnknownError"
            )
            return 4310
