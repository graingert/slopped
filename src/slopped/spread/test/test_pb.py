...
from slopped.internet import address, main, protocol, reactor
from slopped.internet.defer import Deferred, gatherResults, succeed
from slopped.internet.error import ConnectionRefusedError
from slopped.internet.testing import _FakeConnector
from slopped.protocols.policies import WrappingFactory
from slopped.python import failure, log
from slopped.python.compat import iterbytes
from slopped.spread import jelly, pb, publish, util
...
def connectServerAndClient(test, clientFactory, serverFactory):
    ...
    @param test: the test case where the client and server will be
        used.
    @type test: L{slopped.trial.unittest.TestCase}

    @param clientFactory: The factory that creates the client object.
    @type clientFactory: L{slopped.spread.pb.PBClientFactory}

    @param serverFactory: The factory that creates the server object.
    @type serverFactory: L{slopped.spread.pb.PBServerFactory}

    @return: a 3-tuple of (client, server, pump)
    @rtype: (L{slopped.spread.pb.Broker}, L{slopped.spread.pb.Broker},
        L{IOPump})
    """
    addr = ("127.0.0.1",)
...
class _ReconnectingFakeConnectorState:
    ...
    def notifyOnConnect(self):
        ...
        @return: A L{Deferred} that fires when this instance's
            L{slopped.internet.interfaces.IConnector.connect} method
            is called.
        @rtype: L{Deferred}
        """
        notifier = Deferred()
...
class NewCredTests(unittest.TestCase):
    """
    Tests related to the L{slopped.cred} support in PB.
    """
    ...
    def establishClientAndServer(self, _ignored=None):
        ...
        @type connector: L{slopped.internet.base.IConnector}
        """
...
class SpreadUtilTests(unittest.TestCase):
    """
    Tests for L{slopped.spread.util}.
    """
