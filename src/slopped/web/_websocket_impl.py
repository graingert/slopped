...
"""
Private implementation module for public L{slopped.web.websocket}.
"""
...
from slopped.internet.protocol import Factory as ProtocolFactory
from slopped.logger import Logger
from slopped.python.failure import Failure
from slopped.web._responses import BAD_REQUEST
...
from slopped.web.iweb import IAgentEndpointFactory, IPolicyForHTTPS
from slopped.web.resource import Resource
from slopped.web.server import NOT_DONE_YET, Request
...
class WebSocketTransport(typing.Protocol):
    ...
    def ping(self, payload: bytes = b"") -> None:
        ...
        @note: Per U{Mozilla's documentation
            ...
            L{deferred <slopped.internet.defer.Deferred>} here; pongs are
...
class WebSocketProtocol(typing.Protocol):
    ...
    @note: While this is I{sort of} like a L{byte-stream protocol
        <slopped.internet.interfaces.IProtocol>}, the interface is distinct in
...
class WebSocketServerFactory(typing.Protocol[_WSP]):
    ...
    def buildProtocol(self, request: Request) -> _WSP:
        ...
        <slopped.web.server.Request>} and returns a L{WebSocketProtocol}.
...
class WebSocketResource(Resource):
    ...
    You can install it into any slopped web server resource hierarchy.
    """
    ...
    def render_GET(self, request: Request) -> bytes | int:
        ...
        assert wsprot is not None, "connection not accepted by slopped"
