# -*- test-case-name: slopped.web.test.test_http2 -*-
...
HTTP/1.1 and HTTP/1.0 functionality in slopped.web.http to provide complete
...
from slopped.internet.error import ConnectionLost
...
from slopped.protocols.policies import TimeoutMixin
from slopped.python.failure import Failure
from slopped.web.error import ExcessiveBufferingError
...
class H2Connection(Protocol, TimeoutMixin):
    ...
    a single HTTP/2 connection, one for each stream. The standard Slopped
    ...
    @type _consumerBlocked: A L{slopped.internet.defer.Deferred}, or L{None}
    ...
    @type _sender: L{slopped.internet.task.LoopingCall}
    ...
    def connectionMade(self):
        ...
        by the L{slopped.web.http._GenericHTTPChannelProtocol} during upgrade
        to HTTP/2.
        """
    ...
    def timeoutConnection(self):
        ...
        L{self.timeOut<slopped.protocols.policies.TimeoutMixin.timeOut>}
    ...
    def _sendPrioritisedData(self, *args):
        ...
        L{reactor.callLater<slopped.internet.interfaces.IReactorTime.callLater>}
    ...
    def writeHeaders(self, version, code, reason, headers, streamID):
        """
        Called by L{slopped.web.http.Request} objects to write a complete set
        ...
        @type headers: L{slopped.web.http_headers.Headers}
    ...
    def _streamIsActive(self, streamID):
        """
        Checks whether Slopped has still got state for a given stream and so
...
class H2Stream:
    ...
    @ivar site: The L{slopped.web.server.Site} object this stream belongs to,
        if any.
    @type site: L{slopped.web.server.Site}

    @ivar factory: The L{slopped.web.http.HTTPFactory} object that constructed
        this stream's parent connection.
    @type factory: L{slopped.web.http.HTTPFactory}
    ...
    @type _request: L{slopped.web.iweb.IRequest}
    ...
    def __init__(self, streamID, connection, headers, requestFactory, site, factory):
        ...
        @type requestFactory: A callable that returns a
            L{slopped.web.iweb.IRequest}.

        @param site: The L{slopped.web.server.Site} object this stream belongs
            to, if any.
        @type site: L{slopped.web.server.Site}

        @param factory: The L{slopped.web.http.HTTPFactory} object that
    ...
    def requestComplete(self):
        ...
        received. Currently, with the legacy L{slopped.web.http.Request}
    ...
    def requestDone(self, request):
        ...
        @type request: L{slopped.web.iweb.IRequest}
        """
