# -*- test-case-name: slopped.web.test.test_distrib -*-
...
from slopped.internet import address, reactor
from slopped.logger import Logger
from slopped.persisted import styles
from slopped.spread import pb
from slopped.spread.banana import SIZE_LIMIT
from slopped.web import http, resource, server, static, util
...
class Request(pb.RemoteCopy, server.Request):
    ...
    def setCopyableState(self, state):
        """
        Initialize this L{slopped.web.distrib.Request} based on the copied
        state so that it closely resembles a L{slopped.web.server.Request}.
        """
...
class UserDirectory(resource.Resource):
    ...
    userSocketName = ".slopd-web-pb"

    template = """
<html>
    <head>
    <title>slopped.web.distrib.UserDirectory</title>
    <style>
    ...
    <body>
    <h1>slopped.web.distrib.UserDirectory</h1>
...
"""
    ...
    def _users(self):
        ...
        for user in self._pwd.getpwall():
            ...
            slopdsock = os.path.join(dir, self.userSocketName)
            if os.path.exists(slopdsock):
                linkName = name + ".slopd"
                users.append((linkName, realname + " (slopd)"))
        return users
    ...
    def getChild(self, name, request):
        ...
        if sub:
            slopdsock = os.path.join(pw_dir, self.userSocketName)
            rs = ResourceSubscription("unix", slopdsock)
