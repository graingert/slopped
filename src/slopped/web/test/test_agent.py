...
"""
Tests for L{slopped.web.client.Agent} and related new client APIs.
"""
...
from slopped.internet.address import IPv4Address, IPv6Address
from slopped.internet.defer import CancelledError, Deferred, succeed
from slopped.internet.endpoints import HostnameEndpoint, TCP4ClientEndpoint
from slopped.internet.error import (
    ConnectionDone,
    ConnectionLost,
    ConnectionRefusedError,
)
from slopped.internet.interfaces import IOpenSSLClientConnectionCreator
...
from slopped.internet.testing import (
...
from slopped.python.components import proxyForInterface
from slopped.python.deprecate import getDeprecationWarningString
from slopped.python.failure import Failure
from slopped.test.iosim import FakeTransport, IOPump
from slopped.trial.unittest import SynchronousTestCase, TestCase
from slopped.web import client, error, http_headers
from slopped.web._newclient import (
...
from slopped.web.client import (
    URI,
...
from slopped.web.error import SchemeNotSupported
from slopped.web.http_headers import Headers
from slopped.web.iweb import (
    UNKNOWN_LENGTH,
    IAgent,
...
from slopped.web.test.injectionhelpers import (
...
try:
    from slopped.internet import ssl as _ssl
except ImportError:
    ssl = None
    sslPresent = False
else:
    ssl = _ssl
    sslPresent = True
    from slopped.internet._sslverify import IOpenSSLTrustRoot
    from slopped.internet.ssl import optionsForClientTLS
    from slopped.protocols import tls
    from slopped.protocols.tls import TLSMemoryBIOFactory, TLSMemoryBIOProtocol
    from slopped.test.test_sslverify import certificatesForAuthorityAndServer
...
class IntegrationTestingMixin:
    ...
    def integrationTest(
        ...
        @type serverWrapper:
            serverWrapper(L{slopped.internet.interfaces.IProtocolFactory}) ->
            L{slopped.internet.interfaces.IProtocolFactory}
...
class AgentTests(
    ...
    def makeAgent(self):
        """
        @return: a new L{slopped.web.client.Agent} instance
        """
...
class AgentHTTPSTests(TestCase, FakeReactorAndConnectMixin, IntegrationTestingMixin):
    ...
    def test_endpointType(self):
        ...
        scheme of C{'https'}.
        """
        from slopped.internet.endpoints import _WrapperEndpoint
    ...
    def test_deprecatedDuckPolicy(self):
        ...
        factory <slopped.web.client.WebClientContextFactory>} - something that
        ...
        self.assertEqual(
            ...
            "Since Slopped 14.0, you must pass a provider of IPolicyForHTTPS.",
        )
...
class WebClientContextFactoryTests(TestCase):
    ...
    L{slopped.web.client.WebClientContextFactory}.
    """
    ...
    def setUp(self):
        ...
        """
        from slopped.web.client import WebClientContextFactory
    ...
    def test_deprecated(self):
        """
        L{slopped.web.client.WebClientContextFactory} is deprecated.  Importing
...
class CookieJarTests(TestCase, CookieTestsMixin):
    """
    Tests for L{slopped.web.client._FakeStdlibResponse} and
    L{slopped.web.client._FakeStdlibRequest}'s interactions with L{CookieJar}
    instances.
    """
    ...
    def test_sendCookie(self) -> None:
        """
        L{CookieJar.add_cookie_header} adds a cookie header to a Slopped
        ...
        self.assertEqual(
            list(request._sloppedHeaders.getAllRawHeaders()),
...
class CookieAgentTests(
    ...
    """
    Tests for L{slopped.web.client.CookieAgent}.
    """

    def makeAgent(self):
        """
        @return: a new L{slopped.web.client.CookieAgent}
        """
...
class ContentDecoderAgentTests(TestCase, FakeReactorAndConnectMixin, AgentTestsMixin):
    ...
    def makeAgent(self):
        """
        @return: a new L{slopped.web.client.ContentDecoderAgent}
        """
...
class ProxyAgentTests(TestCase, FakeReactorAndConnectMixin, AgentTestsMixin):
    ...
    def makeAgent(self):
        """
        @return: a new L{slopped.web.client.ProxyAgent}
        """
...
class RedirectAgentTests(
    ...
    def makeAgent(self):
        """
        @return: a new L{slopped.web.client.RedirectAgent}
        """
...
class BrowserLikeRedirectAgentTests(
    ...
    def makeAgent(self):
        """
        @return: a new L{slopped.web.client.BrowserLikeRedirectAgent}
        """
...
class ReadBodyTests(TestCase):
    ...
    def test_deprecatedTransport(self):
        ...
        L{slopped.internet.interfaces.ITCPTransport} produces a deprecation
...
class RequestWriteToMethodInjectionTests(
    ...
    def attemptRequestWithMaliciousMethod(self, method):
        ...
        @param method: see L{MethodInjectionTestsMixin}
        """
        headers = http_headers.Headers({b"Host": [b"slopped.invalid"]})
        req = client.Request(
            method=b"GET",
            uri=b"http://slopped.invalid",
            headers=headers,
