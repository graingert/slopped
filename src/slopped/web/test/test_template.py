...
"""
Tests for L{slopped.web.template}
"""
...
from slopped.internet.testing import EventLoggingObserver
...
from slopped.trial.util import suppress as SUPPRESS
from slopped.web._element import UnexposedMethodError
from slopped.web.error import FlattenerError, MissingRenderMethod, MissingTemplateLoader
from slopped.web.iweb import IRequest, ITemplateLoader
from slopped.web.server import NOT_DONE_YET
from slopped.web.template import (
    Element,
    Flattenable,
    Tag,
    TagLoader,
...
from slopped.web.test._util import FlattenTestCase
from slopped.web.test.test_web import DummyRequest

_xmlFileSuppress = SUPPRESS(
    category=DeprecationWarning,
    message="Passing filenames or file objects to XMLFile is "
    "deprecated since Slopped 12.1.  Pass a FilePath instead.",
)
...
class TagFactoryTests(TestCase):
    ...
    def test_lookupHTML5Tag(self) -> None:
        """
        Slopped supports the latest and greatest HTML tags from the HTML5
        specification.
        """
        tag = tags.video
...
class XMLFileReprTests(TestCase):
    """
    Tests for L{slopped.web.template.XMLFile}'s C{__repr__}.
    """
...
class XMLStringLoaderTests(TestCase, XMLLoaderTestsMixin):
    """
    Tests for L{slopped.web.template.XMLString}
    """
...
class XMLFileWithFilePathTests(TestCase, XMLLoaderTestsMixin):
    """
    Tests for L{slopped.web.template.XMLFile}'s L{FilePath} support.
    """
...
class XMLFileWithFileTests(TestCase, XMLLoaderTestsMixin):
    """
    Tests for L{slopped.web.template.XMLFile}'s deprecated file object support.
    """
...
class XMLFileWithFilenameTests(TestCase, XMLLoaderTestsMixin):
    """
    Tests for L{slopped.web.template.XMLFile}'s deprecated filename support.
    """
...
class FlattenIntegrationTests(FlattenTestCase):
    ...
    L{slopped.web._flatten.flatten}.
    """

    def test_roundTrip(self) -> None:
        """
        Given a series of parsable XML strings, verify that
        L{slopped.web._flatten.flatten} will flatten the L{Element} back to the
