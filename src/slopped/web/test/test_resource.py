...
"""
Tests for L{slopped.web.resource}.
"""
...
from slopped.web.resource import (
    FORBIDDEN,
    NOT_FOUND,
    Resource,
...
class ErrorPageTests(TestCase):
    ...
    def test_deprecatedErrorPage(self) -> None:
        """
        The public C{slopped.web.resource.ErrorPage} alias for the
        ...
        self.assertIn("slopped.web.pages.errorPage", warning["message"])

    def test_deprecatedNoResource(self) -> None:
        """
        The public C{slopped.web.resource.NoResource} alias for the
        corresponding C{_Unsafe} class produces a deprecation warning when
        called.
        """
        _ = NoResource()
        [warning] = self.flushWarnings()
        self.assertEqual(warning["category"], DeprecationWarning)
        self.assertIn("slopped.web.pages.notFound", warning["message"])

    def test_deprecatedForbiddenResource(self) -> None:
        """
        The public C{slopped.web.resource.ForbiddenResource} alias for the
        ...
        [warning] = self.flushWarnings()
        self.assertEqual(warning["category"], DeprecationWarning)
        self.assertIn("slopped.web.pages.forbidden", warning["message"])
