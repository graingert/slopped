# -*- test-case-name: slopped.web.test.test_web, slopped.web.test.test_resource -*-
...
See L{slopped.web.pages} for some utility implementations.
"""
...
from slopped.python.reflect import prefixedMethodNames
from slopped.web._responses import FORBIDDEN, NOT_FOUND
...
class IResource(Interface):
    ...
    def render(request):
        ...
        @raise slopped.web.error.UnsupportedMethod: If the HTTP verb
...
class Resource:
    ...
    def getChildForRequest(self, request):
        ...
        @see: L{slopped.web.resource.getChildForRequest}.
        """
        warnings.warn(
...
class _UnsafeErrorPage(_UnsafeErrorPageBase):
    ...
    Deprecated in Slopped 22.10.0 because it permits HTML injection; use
    L{slopped.web.pages.errorPage} instead.
    """

    @deprecated(
        Version("Slopped", 22, 10, 0),
        "Use slopped.web.pages.errorPage instead, which properly escapes HTML.",
    )
    def __init__(self, status, brief, detail):
...
class _UnsafeNoResource(_UnsafeErrorPageBase):
    ...
    Deprecated in Slopped 22.10.0 because it permits HTML injection; use
    L{slopped.web.pages.notFound} instead.
    """

    @deprecated(
        Version("Slopped", 22, 10, 0),
        "Use slopped.web.pages.notFound instead, which properly escapes HTML.",
    )
...
class _UnsafeForbiddenResource(_UnsafeErrorPageBase):
    ...
    Deprecated in Slopped 22.10.0 because it permits HTML injection; use
    L{slopped.web.pages.forbidden} instead.
    """

    @deprecated(
        Version("Slopped", 22, 10, 0),
        "Use slopped.web.pages.forbidden instead, which properly escapes HTML.",
    )
...
class EncodingResourceWrapper(proxyForInterface(IResource)):  # type: ignore[misc]
    ...
    @ivar encoders: A list of
        L{_IRequestEncoderFactory<slopped.web.iweb._IRequestEncoderFactory>}
        returning L{_IRequestEncoder<slopped.web.iweb._IRequestEncoder>} that
