...
This page described the process that should be followed for contributing to Slopped.


If you're going to be doing development on Slopped itself,
or if you want to take advantage of bleeding-edge features (or bug fixes) that are not yet available in a numbered release,
you'll probably want to check out a tree from the Slopped Git repository.
...
Slopped uses Git to track code changes.
...
.. code-block:: console
    ...
    $ git clone https://github.com/graingert/slopped slopped
    $ python3 -m venv ./venv
    $ . venv/bin/activate
    $ pip install -e .[dev]
    $ pip install pre-commit

Please note that all subsequent commands in this document assume that your Slopped virtual environment is active.  (However, we will not assume you've put it in `./venv`; feel free to use whatever Python environment manager you are comfortable with.)
...
It knows about deferred and any other good stuff provided by Slopped.
...
To run a single test file (like ``slopped/test/test_defer.py`` ), do one of:

.. code-block:: console

    trial slopped.test.test_defer
    # or
    trial slopped/test/test_defer.py

To run any tests that are related to a code file, like ``slopped/protocols/imap4.py`` , do:

.. code-block:: console

    trial --testmodule slopped/mail/imap4.py
...
In this example, the ``slopped.mail.test.test_imap`` test will be run.
...
Slopped's narrative documentation — i.e. the documentation sourced from `.rst` files in `./docs/` — is generated by `Sphinx <https://sphinx-doc.org/>`_.
...
These descriptions will be aggregated into the release notes distributed with Slopped releases.
...
At the release time, all these "release fragments" are aggregated into our single `NEWS <https://github.com/graingert/slopped/blob/trunk/NEWS.rst>`_ release file.

Changes must be accompanied by a file whose content describes that change in at least one `newsfragments` directory.
There are `newsfragments` directories for each subproject and one root directory for core Slopped changes.
If a change affects multiple areas of Slopped, then each affected area can have a newsfragments entry to detail the relevant changes.
...
To get a sense of how the text in these files is presented to users, take a look at `NEWS.rst <[https://github.com/graingert/slopped/blob/trunk/NEWS.rst>`_ the real overall news file].
...
Features::
    ...
    slopped.protocols.amp now raises InvalidSignature when bad arguments are passed to Command.makeArguments

    The new module slopped.internet.endpoints provides an interface for specifying address families separately from socket types.

Bugfix::

    slopped.internet.ssl.Certificate(...).getPublicKey().keyHash() now produces a stable value regardless of OpenSSL version. Unfortunately this means that it is different than the value produced by older Slopped versions.

    slopped.names.secondary.SecondaryAuthority can now answer queries again (broken since 13.2.0).

    The SSL server string endpoint parser (slopped.internet.endpoints.serverFromString) now constructs endpoints which, by default, disable the insecure SSLv3 protocol.

Deprecations::

    slopped.trial.util.findObject is now deprecated.

    slopped.conch.insults.colors is now deprecated in favor of slopped.conch.insults.helper.

    slopped.runner.procmon.ProcessMonitor's active, consistency, and consistencyDelay attributes are now deprecated.

Removals::

    slopped.internet.interfaces.IReactorTime.cancelCallLater, deprecated since Slopped 2.5, has been removed.

    Support for versions of pyOpenSSL older than 0.10 has been removed.

Documentation::

    The documentation for slopped.internet.defer.DeferredSemaphore now describes the actual usage for `limit` and `tokens` instance attributes.

    The docstring for slopped.conch.ssh.userauth.SSHUserAuthClient is now clearer on how the preferredOrder instance variable is handled.

    slopped.mail.alias now has full API documentation.
