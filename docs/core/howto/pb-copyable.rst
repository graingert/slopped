...
simply copying the contents of an object to a remote process (:py:class:`pb.Copyable <slopped.spread.pb.Copyable>` ). The second covers how
to copy those contents once, then update them later when they change (:py:class:`Cacheable <slopped.spread.pb.Cacheable>` ).
...
return values of a :py:meth:`callRemote <slopped.spread.pb.RemoteReference.callRemote>` function. However,
...
type, or another :py:class:`pb.Referenceable <slopped.spread.pb.Referenceable>` object. At some point you want
...
you'll encounter the dreaded :py:class:`InsecureJelly <slopped.spread.jelly.InsecureJelly>` exception. This is
Slopped's way of telling you that you've violated a security restriction,
...
with the :py:func:`setUnjellyableForClass <slopped.spread.jelly.setUnjellyableForClass>` function  [#]_ .
...
dictionary value]), it must inherit from one of the four :py:class:`Serializable <slopped.spread.pb.Serializable>` classes. In this section,
we focus on :py:class:`Copyable <slopped.spread.pb.Copyable>` .
...
string. The object's :py:meth:`getStateToCopy <slopped.spread.pb.Copyable.getStateToCopy>` method is
used to get the state: this is provided by :py:class:`pb.Copyable <slopped.spread.pb.Copyable>` , and the default just retrieves 
...
created by theirs). It also inherits from :py:class:`pb.RemoteCopy <slopped.spread.pb.RemoteCopy>` , which is a requirement for all
...
:py:meth:`setCopyableState <slopped.spread.pb.RemoteCopy.setCopyableState>` method.
...
.. code-block:: console
    ...
    [-] slopped.spread.pb.PBServerFactory starting on 8800
    [-] Starting factory <slopped.spread.pb.PBServerFactory instance at
    0x406159cc>
...
- The first argument to ``setUnjellyableForClass`` must refer
  ...
  following them.
- The class that is sent must inherit from :py:class:`pb.Copyable <slopped.spread.pb.Copyable>` . The class that is registered to
  receive it must inherit from :py:class:`pb.RemoteCopy <slopped.spread.pb.RemoteCopy>`  [#]_ . 
...
- The class that is derived from :py:class:`pb.RemoteCopy <slopped.spread.pb.RemoteCopy>` will be created using a
  constructor ``__init__`` method that takes no arguments. All
  setup must be performed in the ``setCopyableState`` method. As
  the docstring on :py:class:`RemoteCopy <slopped.spread.pb.RemoteCopy>` says, don't implement a
...
- ``pb.Copyable`` is mostly implemented
  in ``slopped.spread.flavors`` , and the docstrings there are
  the best source of additional information.
- ``Copyable`` is also used in :py:mod:`slopped.web.distrib` to deliver HTTP requests to other
...
or changes in state whenever it is modified. The :py:class:`pb.Cacheable <slopped.spread.pb.Cacheable>` class provides a framework to
implement this.




:py:class:`pb.Cacheable <slopped.spread.pb.Cacheable>` is derived
from :py:class:`pb.Copyable <slopped.spread.pb.Copyable>` , so it is
...
have an object "publishing" on the sending side (derived from :py:class:`pb.Cacheable <slopped.spread.pb.Cacheable>` ), matched with one"observing" on the receiving side (derived from :py:class:`pb.RemoteCache <slopped.spread.pb.RemoteCache>` ).
...
add two methods: :py:meth:`getStateToCacheAndObserveFor <slopped.spread.pb.Cacheable.getStateToCacheAndObserveFor>`
and :py:meth:`stoppedObserving <slopped.spread.pb.Cacheable.stoppedObserving>` . The first
...
On the receiver end, you make your cache class inherit from :py:class:`pb.RemoteCache <slopped.spread.pb.RemoteCache>` , and implement the 
...
announcing every change that takes place. The :py:meth:`stoppedObserving <slopped.spread.pb.Cacheable.stoppedObserving>` method is
...
- The best source for information comes from the docstrings
  in :py:mod:`slopped.spread.flavors` ,
  where ``pb.Cacheable`` is implemented.
- The :py:mod:`spread.publish <slopped.spread.publish>` module also
...
.. [#] Note that, in this context, "unjelly"  is
       ...
       In particular, "unjellyable"  does *not*  mean "cannot be jellied" . :py:class:`Unpersistable <slopped.spread.jelly.Unpersistable>`  means "not persistable" , but "unjelly" , "unserialize" , and "unpickle" 
       mean to reverse the operations of "jellying" , "serializing" , and
       "pickling" .
.. [#] :py:class:`pb.RemoteCopy <slopped.spread.pb.RemoteCopy>`  is actually defined
         in :py:mod:`slopped.spread.flavors` , but
...
.. [#] This is actually a :py:class:`RemoteCacheObserver <slopped.spread.pb.RemoteCacheObserver>` , but it isn't very
...
.. [#] This applies to
         multiple references through the same :py:class:`Broker <slopped.spread.pb.Broker>` . If you've managed to make multiple
