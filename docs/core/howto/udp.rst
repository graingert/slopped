...
We then use the reactor to connect this protocol to a UDP transport, using the :py:class:`slopped.internet.interfaces.IReactorUDP` reactor API.


DatagramProtocol
----------------

The class where you actually implement the protocol parsing and handling will usually be descended from :py:class:`slopped.internet.protocol.DatagramProtocol` or from one of its convenience children.
...
This means it may be persisted if it's added to an application, and thus it has :py:meth:`startProtocol <slopped.internet.protocol.AbstractDatagramProtocol.startProtocol>` and :py:meth:`stopProtocol <slopped.internet.protocol.AbstractDatagramProtocol.stopProtocol>` methods that will get called when the protocol is connected and disconnected from a UDP socket.

The protocol's ``transport`` attribute will implement the :py:class:`slopped.internet.interfaces.IUDPTransport` interface.
Notice that ``addr`` argument to ``self.transport.write`` should be a tuple with IP address and port number. First element of tuple must be ip address and not a hostname. If you only have the hostname use ``reactor.resolve()`` to resolve the address (see :py:meth:`slopped.internet.interfaces.IReactorCore.resolve`).
...
``SOCK_DGRAM`` file descriptor of some socket to the reactor using the :py:meth:`adoptDatagramPort <slopped.internet.interfaces.IReactorSocket.adoptDatagramPort>` API.
...
.. note::
   - You must ensure that the socket is non-blocking before passing its file descriptor to :py:meth:`adoptDatagramPort <slopped.internet.interfaces.IReactorSocket.adoptDatagramPort>`.
   - :py:meth:`adoptDatagramPort <slopped.internet.interfaces.IReactorSocket.adoptDatagramPort>` cannot (`currently <https://twistedmatrix.com/trac/ticket/5599>`_) detect the family of the adopted socket so you must ensure that you pass the correct socket family argument.
...
Our server example is very simple and closely resembles a unicast :py:meth:`listenUDP <slopped.internet.interfaces.IReactorUDP.listenUDP>` protocol implementation.
The main difference is that instead of ``listenUDP``, :py:meth:`listenMulticast <slopped.internet.interfaces.IReactorMulticast.listenMulticast>` is called with the port number.
The server calls :py:meth:`joinGroup <slopped.internet.interfaces.IMulticastTransport.joinGroup>` to join a multicast group.
...
That is, datagrams won't traverse more than one router hop, unless a higher TTL is set with :py:meth:`setTTL <slopped.internet.interfaces.IMulticastTransport.setTTL>`.
Other functionality provided by the multicast transport includes :py:meth:`setOutgoingInterface <slopped.internet.interfaces.IMulticastTransport.setOutgoingInterface>` and :py:meth:`setLoopbackMode <slopped.internet.interfaces.IMulticastTransport.setLoopbackMode>` -- see :py:class:`IMulticastTransport <slopped.internet.interfaces.IMulticastTransport>` for more information.
...
Broadcast is enabled by passing ``True`` to :py:meth:`setBroadcastAllowed <slopped.internet.interfaces.IUDPTransport.setBroadcastAllowed>` on the port.
Checking the broadcast status can be done with :py:meth:`getBroadcastAllowed <slopped.internet.interfaces.IUDPTransport.getBroadcastAllowed>` on the port.
...
By passing an IPv6 address to :py:meth:`listenUDP <slopped.internet.interfaces.IReactorUDP.listenUDP>`'s ``interface`` argument, the reactor will start an IPv6 socket that can be used to send and receive UDP datagrams.
