...
Slopped Web provides support for the WebSocket protocol for clients (such as web browsers) to connect to resources and receive bi-directional communication with the server.
...
.. note::
   ...
   In order to use the websocket support in Slopped, you will need the ``websocket`` optional dependency, so :doc:`install Slopped </installation>` with either ``pip install slopped[websocket]`` or install one that includes it, such as ``slopped[all_non_platform]`` or ``slopped[all]`` .

WebSocket Server
----------------

Let's use Slopped to create a simple websocket server, and then build a web-browser based client to communicate with it.

To begin with, we will need a folder with 3 files in it.
First, let's do the Slopped part.  We need a :py:class:`slopped.web.websocket.WebSocketResource` to be served at a known URL, so let's put one into a ``.rpy`` file called ``websocket-server.rpy`` :
...
Note that by using a ``@classmethod`` for ``buildProtocol``, the *type* of  ``WebSocketDemo`` complies with the :py:class:`slopped.web.websocket.WebSocketServerFactory` :py:class:`protocol <typing.Protocol>`, returning a ``WebSocketDemo`` that complies with :py:class:`slopped.web.websocket.WebSocketProtocol` ; we can then pass the type of ``WebSocketDemo`` itself, without instantiating it, to :py:class:`slopped.web.websocket.WebSocketResource`.
...
If you were to put these all into a folder called ``my-websocket-demo`` , you can use ``slop`` to serve them, via ``slop web --path ./my-websocket-demo``.
...
And that's all you need to implement a websocket server with Slopped!

Since :py:class:`slopped.web.websocket.WebSocketResource` is a standard Slopped :py:class:`resource <slopped.web.server.Resource>`, you can integrate it with things like :doc:`authentication <http-auth>` or :doc:`sessions <session-basics>`, just as you would any other resource.

WebSocket Client
----------------

Of course, if we have a server, we may also want to talk to it from our Slopped applications.
To do that, let's build a simple websocket client, with :py:class:`slopped.web.websocket.WebSocketClientEndpoint`.
This client could talk to **any** WebSocket server, regardless of how it was implemented, but since we just built one with Slopped, we'll use that one.
...
.. note::
   ...
   Obtaining a valid HTTPS certificate for your local example goes a bit beyond the scope of this tutorial, but you can test using the ``wss://`` protocol, as well as testing against a non-Slopped server, by substituting the URL ``"wss://echo.websocket.org/"`` in the client demo.
   Make sure to also install the ``[tls]`` :ref:`optional dependency group <Optional Dependencies>` to install the required dependencies for Slopped's HTTPS client.
...
And now you have a functioning WebSocket server and client using Slopped!
