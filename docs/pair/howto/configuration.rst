...
Slopped Pair: Device Configuration
==================================
...
Slopped Pair's Test Suite
-------------------------


    

Certain system configuration is required before the full Slopped Pair test suite can be run.
Without this setup the test suite will lack the permission necessary to access tap and tun devices.
Some tests will still run but the integration tests which verify Slopped Pair can successfully read from and write to real devices will be skipped.
...
Run it to configure your system so that you can perform a complete run of the Slopped Pair test suite.
...
These addresses are hard-coded into the Slopped Pair test suite so this problem is not easily avoided.
...
If you encounter timeouts when running the Slopped Pair test suite then this may apply to you.
