...
Slopped Pair: Tunnels And Network Taps
======================================
On Linux, Slopped Pair supports the special *tun* and *tap* network interface types.
...
This document will not cover the details of these platform-provided features, but it will explain how to use the Slopped Pair APIs which interact with them.
...
Tuntap Ports
------------
The :py:class:`slopped.pair.tuntap.TuntapPort` class is the entry point into the tun/tap functionality.
...
If the protocol provides :py:class:`slopped.pair.ethernet.IEthernetProtocol` then it is associated with a tap device.
Otherwise the protocol must provide :py:class:`slopped.pair.raw.IRawPacketProtocol` and it will be associated with a tun device.
...
However, it should give you some idea of how tun/tap functionality fits into a Slopped application.
...
Both ``IEthernetProtocol`` and ``IRawSocketProtocol`` are similar to :py:class:`slopped.internet.protocol.DatagramProtocol` .
